...
def build_month_code_detail_from_minors(minors, date_from, date_to):
    cols = ["部位大类", "物料号", "产量(kg)", "含税金额", "月均价"]
    if minors is None or getattr(minors, "empty", True):
        return pd.DataFrame(columns=cols)
    ...
    m = minors.copy()
    if "日期" not in m.columns:
        return pd.DataFrame(columns=cols)
    m["日期"] = pd.to_datetime(m["日期"], errors="coerce").dt.normalize()
    m = m.loc[(m["日期"] >= date_from.normalize()) & (m["日期"] <= date_to.normalize())].copy()
    if m.empty:
        return pd.DataFrame(columns=cols)

    if "物料号" not in m.columns and "子类" in m.columns:
        m = m.rename(columns={"子类": "物料号"})
    if "部位大类" not in m.columns and "项目" in m.columns:
        m = m.rename(columns={"项目": "部位大类"})
    missing = [c for c in ["部位大类", "物料号", "产量(kg)", "含税金额"] if c not in m.columns]
    if missing:
        return pd.DataFrame(columns=cols)

    m["部位大类"] = m["部位大类"].astype(str).str.strip()
    m["物料号"] = m["物料号"].astype(str).str.strip()
    m["产量(kg)"] = pd.to_numeric(m["产量(kg)"], errors="coerce").fillna(0.0)
    m["含税金额"] = pd.to_numeric(m["含税金额"], errors="coerce").fillna(0.0)
    detail = (
        m.groupby(["部位大类", "物料号"], as_index=False)[["产量(kg)", "含税金额"]]
         .sum(min_count=1)
    )
    detail["月均价"] = np.where(
        detail["产量(kg)"] != 0,
        detail["含税金额"] / detail["产量(kg)"],
        np.nan,
    )
    return detail[cols]
...
if overview is None or overview.empty:
    ...
    with st.expander("累计（本月至所选日）", expanded=True):
        ...
        # —— 当月产量/金额直接汇总已还原后的子类，再按部位汇总 ——
        cum_base = pd.DataFrame()
        try:
            if minors is not None and not minors.empty:
                detail = build_month_code_detail_from_minors(minors, month_start, sel_dt)
                if not detail.empty:
                    detail = detail.rename(columns={"部位大类": "项目"})
        ...
        if minors is not None and not minors.empty:
            try:
                code_detail = build_month_code_detail_from_minors(minors, month_start, sel_dt)
                if not code_detail.empty:
                    if code2desc:
