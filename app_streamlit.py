...
def apply_chest_cartilage_theoretical_restoration(
    m_split,
    m_code_daily,
    ...
    if "物料号" not in rules.columns or "胸软骨标准产成率" not in rules.columns:
        return m_split.copy()
    rules["物料号"] = rules["物料号"].astype(str).str.strip()
    rules["胸软骨标准产成率"] = _pd.to_numeric(rules["胸软骨标准产成率"], errors="coerce")
    rules = rules.dropna(subset=["物料号", "胸软骨标准产成率"])
    rules = rules[rules["胸软骨标准产成率"] > 0]
    if rules.empty:
        return m_split.copy()
    ...
    source_daily = source_daily.merge(rules[["物料号", "胸软骨标准产成率"]], on="物料号", how="left")
    source_daily = source_daily.dropna(subset=["胸软骨标准产成率"])
    if source_daily.empty:
        return m_split.copy()

    separated = (
        base[base["物料号"] == CHEST_CARTILAGE_CODE]
        .groupby("日期", as_index=False)["产量(kg)"].sum()
        .rename(columns={"产量(kg)": "_已分切胸软骨量"})
    )

    lw_map = dict(zip(lw["日期"], lw["毛鸡净重(kg)"]))
    price_map = dict(zip(prices.get("日期", []), prices.get("胸软骨单价", [])))
    separated_map = dict(zip(separated["日期"], separated["_已分切胸软骨量"])) if not separated.empty else {}
    ...
    for d, g in source_daily.groupby("日期"):
        if pd.isna(d) or d not in lw_map:
            continue
        ...
        if source_total_qty <= 0:
            continue
        rates = _pd.to_numeric(g["胸软骨标准产成率"], errors="coerce").fillna(0.0)
        standard_rate = float(_np.average(rates, weights=source_qty)) if source_qty.sum() > 0 else float(rates.mean())
        theoretical_qty = float(lw_map[d]) * standard_rate
        separated_qty = float(separated_map.get(d, 0.0) or 0.0)
        pending_qty = max(theoretical_qty - separated_qty, 0.0)
        if pending_qty <= 0:
            continue
        ...
        if pd.isna(price):
            ...
            continue
        pending_amt = pending_qty * float(price) * CHEST_CARTILAGE_TAX_FACTOR
        for _, row in g.iterrows():
            row_qty = float(row["产量(kg)"])
            share = row_qty / source_total_qty if source_total_qty > 0 else 0.0
            ...
            new_rows.append({
                ...
                "项目": "骨架类",
                "物料号": code,
                "产量(kg)": row_qty - pending_qty * share,
                "含税金额": float(row["含税金额"]) - pending_amt * share,
            })
        new_rows.append({
            "日期": d,
            "项目": "胸类-胸",
    ...
    if missing_price_days:
        try:
            ...
            st.warning(f"胸软骨物料 {CHEST_CARTILAGE_CODE} 缺少价格，以下日期未执行胸软骨理论还原：{day_text}")
        except Exception:
            pass
...
def build_overview(xls, code2major, df_lw=None, manual_month_df=None, alloc_long=None, chest_cartilage_rules=None):
    ...
    if chest_cartilage_rules is not None and not getattr(chest_cartilage_rules, "empty", True):
        ...
        m_split = apply_chest_cartilage_theoretical_restoration(
            m_split,
            m,
            chest_cartilage_rules,
            df_lw,
...
def read_chest_cartilage_standard_yield(uploaded):
    ...
    读取胸软骨标准产成率表：
    ['来源','物料号','胸软骨标准产成率']。
    兼容旧列名“胸软骨占比”，但按标准产成率参与理论倒推。
    """
    import pandas as pd, io as _io

    cols_out = ["来源", "物料号", "胸软骨标准产成率"]
    if uploaded is None:
    ...
    for source, df in frames:
        ...
        ycol = next((c for c in yield_cols if c in df.columns), None)
        ...
        for _, r in df.iterrows():
            ...
            if not code_values:
                continue
            rate = _ratio_value(r.get(ycol))
            if rate is None:
                continue
            rows.append({"来源": source, "物料号": code_values[0], "胸软骨标准产成率": float(rate)})

    out = pd.DataFrame(rows, columns=cols_out)
    if out.empty:
        return out
    return (
        out.groupby(["来源", "物料号"], as_index=False)["胸软骨标准产成率"]
        .mean()
        .sort_values(["来源", "物料号"])
...
chest_cartilage_yield_file = st.sidebar.file_uploader("胸软骨标准产成率表（可选上传）", type=["xlsx","xls","csv"])
...
chest_cartilage_rules = read_chest_cartilage_standard_yield(chest_cartilage_yield_file)
