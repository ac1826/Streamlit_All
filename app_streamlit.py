...
if overview is None or overview.empty:
    ...
    try:
        ...
        # 供核心指标概览使用的全量销量（不依赖部位映射）
        pr_raw_for_summary = None
        try:
            pr_raw_for_summary = build_daily_code_price_raw(xls)
            if pr_raw_for_summary is not None and not getattr(pr_raw_for_summary, "empty", True):
                pr_raw_for_summary = pr_raw_for_summary.copy()
                pr_raw_for_summary["日期"] = pd.to_datetime(pr_raw_for_summary["日期"], errors="coerce").dt.normalize()
        except Exception:
            pr_raw_for_summary = None
        ...
        def _summary_row(label, start, end):
            ...
            # 总计-销量：全量销量口径，但排除组合汇总行，避免重复计入
            # 总计-销量：使用全量销量口径（销售净重 + 调理品数量取绝对值），不依赖部位映射
            if pr_raw_for_summary is not None and not pr_raw_for_summary.empty:
                _mask = (pr_raw_for_summary["日期"] >= start) & (pr_raw_for_summary["日期"] <= end)
                tot_sale = pd.to_numeric(pr_raw_for_summary.loc[_mask, "数量"], errors="coerce").sum(min_count=1)
            else:
                tot_sale = np.nan
    ...
    with st.expander("累计（本月至所选日）", expanded=True):
        ...
        # 总计-销量：全量销量口径，但排除组合汇总行避免重复
        if '销量(kg)' in cum_base.columns:
            tot_sale = cum_base.loc[~cum_base["项目"].isin(["胸类","鸡头+鸡脖+骨架"]), '销量(kg)'].sum(min_count=1)
        else:
            tot_sale = np.nan
...
try:
    ...
    if overview is None or (hasattr(overview, "empty") and overview.empty):
        ...
        def _read_split_table(uploaded):
            """读取“部位分摊表”，返回 code -> [(part, weight), ...]
            规则：行内部位列归一化；同一物料号多行比例归一化。
            """
            mapping = {}
            if uploaded is None:
                return mapping
            try:
                ...
                for df in frames:
                    cols = [str(c).strip() for c in df.columns]
                    code_col = next((c for c in cols if any(k in c for k in ["物料号","物料编码","物料编号","物料代码","编码","Material","品项","物料"])), None)
                    ratio_col = next((c for c in cols if any(k in c for k in ["比例","权重","占比","比重","ratio","Ratio","RATE"])), None)
                    if not code_col:
                        continue
                    part_cols = [c for c in cols if c not in {code_col, ratio_col}]
                    if not part_cols:
                        continue
                    sub = df[[code_col] + ([ratio_col] if ratio_col else []) + part_cols].copy()
                    ratio_sum_by_code = {}
                    for _, row in sub.iterrows():
                        ...
                        code = codes[0]
                        try:
                            ratio = float(row[ratio_col]) if ratio_col else 1.0
                        ...
                        for pcol in part_cols:
                            ...
                            if w <= 0:
                                continue
                            items.append((_unify(pcol), w))
                            row_sum += w
                        if not items:
                            continue
                        if row_sum > 0:
                            norm_items = [(p, w / row_sum) for p, w in items]
                        else:
                            norm_items = items
                        mapping.setdefault(code, []).append({"ratio": ratio, "items": norm_items})
                        ratio_sum_by_code[code] = ratio_sum_by_code.get(code, 0.0) + ratio
                    for code, rows in list(mapping.items()):
                        if not isinstance(rows, list) or len(rows) == 0:
                            continue
                        rsum = ratio_sum_by_code.get(code, 0.0)
                        if rsum > 0:
                            for r in rows:
                                if isinstance(r, dict) and "ratio" in r:
                                    r["ratio"] = r["ratio"] / rsum
                return mapping
            except Exception:
                return {}
        ...
        split_table_file = st.file_uploader(
            "部位分摊表（可选上传）",
            type=["xlsx","xls","csv"],
            help="宽表：物料号 + 可选比例 + 各部位列（数值为分摊比例，如 0.4）。",
            key="split_table_file"
        )
        split_table = _read_split_table(split_table_file)
        if split_table_file is not None:
            msg = f"已读取 {len(split_table)} 个物料号的部位分摊。" if split_table else "未在部位分摊表中识别到有效的列/数据。"
            st.caption(msg)
        ...
        if not restore_mapping:
            ...
            st.stop()
        if split_table:
            split_codes = set(split_table.keys())
            restore_codes = set(restore_mapping.keys())
            missing_codes = sorted(split_codes - restore_codes)
            if missing_codes:
                st.warning(f"部位分摊表中有 {len(missing_codes)} 个物料号未在物料还原映射表中，相关分摊量不会进入组合还原。")
        ...
        def _calc_for_day(day_value, need_detail=False):
            ...
            if split_table:
                expanded = []
                for _, r in minors_day.iterrows():
                    code_val = r.get(code_col, "")
                    canon_code = _canon_code_val(code_val)
                    qty = float(r.get("产量(kg)", 0.0)) if pd.notna(r.get("产量(kg)", 0.0)) else 0.0
                    if canon_code in split_table and qty != 0:
                        for entry in split_table.get(canon_code, []):
                            if not isinstance(entry, dict):
                                continue
                            ratio = float(entry.get("ratio", 1.0))
                            for part, w in entry.get("items", []):
                                split_qty = qty * ratio * w
                                if need_detail:
                                    split_detail_rows.append({
                                        "日期": day_value,
                                        "物料号": code_val,
                                        "原产量(kg)": qty,
                                        "行比例": ratio,
                ...
                minors_day = pd.DataFrame(expanded)
            ...
            return inc_map, removed_map, detail_rows, lw_day, ov_day, (split_detail_rows if need_detail else [])
        ...
        if split_detail_ref:
            try:
                split_df = pd.DataFrame(split_detail_ref)
                if not split_df.empty:
                    st.markdown("### 部位分摊明细（当日）")
        ...
        try:
            ...
            removed_month = {}
            for d in days_range:
                inc_d, rem_d, _, _, _, sd = _calc_for_day(d, need_detail=bool(split_table))
            ...
            if not over_month.empty:
                ...
                if show_projects_m:
                    ...
                    if not month_summary.empty:
                        ...
                        if split_detail_month:
                            try:
                                split_df_m = pd.DataFrame(split_detail_month)
                                if not split_df_m.empty:
                                    st.markdown("### 部位分摊明细（月累计）")
                                    st.dataframe(split_df_m, use_container_width=True)
                            except Exception:
                                pass
        except Exception:
            pass
