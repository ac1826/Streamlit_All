...
def build_chest_cartilage_price_by_date(pr_raw, dates, manual_month=None, xls=None):
    import pandas as _pd
    import numpy as _np

    if dates is None:
        return _pd.DataFrame(columns=["日期", "胸软骨单价"])
    date_s = _pd.to_datetime(_pd.Series(list(dates)), errors="coerce").dt.normalize().dropna().drop_duplicates()
    if date_s.empty:
        return _pd.DataFrame(columns=["日期", "胸软骨单价"])

    need = _pd.DataFrame({"日期": date_s, "物料号": CHEST_CARTILAGE_CODE})
    price = need.copy()
    price["胸软骨单价"] = _np.nan
    ...
    if manual_month is not None and not getattr(manual_month, "empty", True):
        ...
        if "物料号" in mm.columns and "手工单价" in mm.columns:
            mm["物料号"] = mm["物料号"].astype(str).str.strip()
            fallback = _pd.to_numeric(
                mm.loc[mm["物料号"] == CHEST_CARTILAGE_CODE, "手工单价"],
                errors="coerce",
            ).dropna()
            if not fallback.empty:
                price["胸软骨单价"] = price["胸软骨单价"].where(
                    price["胸软骨单价"].notna(),
                    float(fallback.iloc[-1]),
                )

    return price[["日期", "胸软骨单价"]]
...
def apply_chest_cartilage_structure_restoration(
    ...
    for d, g in source_daily.groupby("日期"):
        ...
        if source_total_qty <= 0:
            continue
        price = price_map.get(d, _np.nan)
        if pd.isna(price):
            missing_price_days.append(d)
            continue
    ...
    if missing_price_days:
        try:
            day_text = "、".join(sorted({pd.Timestamp(d).strftime("%Y-%m-%d") for d in missing_price_days}))
            st.warning(f"胸软骨物料 {CHEST_CARTILAGE_CODE} 缺少价格，以下日期未执行胸软骨占比还原：{day_text}")
        except Exception:
            pass
