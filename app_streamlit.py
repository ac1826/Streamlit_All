...
def apply_chest_cartilage_structure_restoration(
    ...
    rules[ratio_col] = _pd.to_numeric(rules[ratio_col], errors="coerce")
    rules = rules.dropna(subset=["物料号", ratio_col])
    rules = rules[rules[ratio_col] > 0]
    if rules.empty:
        return m_split.copy()
    ...
    source_daily = source_daily.merge(rules[["物料号", ratio_col]], on="物料号", how="left")
    source_daily = source_daily.dropna(subset=[ratio_col])
    if source_daily.empty:
        return m_split.copy()
    ...
    for d, g in source_daily.groupby("日期"):
        ...
        for _, row in g.iterrows():
            ...
            restored_qty = row_qty * ratio
            restored_amt = restored_qty * float(price) * CHEST_CARTILAGE_TAX_FACTOR
            ...
            new_rows.append({
                ...
                "项目": "骨架类",
                "物料号": code,
                "产量(kg)": row_qty - restored_qty,
                "含税金额": float(row["含税金额"]) - restored_amt,
            })
            new_rows.append({
                "日期": d,
                "项目": "胸类-胸",
...
def read_chest_cartilage_structure_ratio(uploaded):
    ...
    ratio_cols = ["胸软骨占比", "胸软骨标准产成率", "标准产成率", "胸软骨产成率", "胸软骨标准率"]
    ...
    for source, df in frames:
        ...
        ycol = next((c for c in ratio_cols if c in df.columns), None)
        ...
        for _, r in df.iterrows():
            ...
            if rate is None:
                continue
            rows.append({"来源": source, "物料号": code_values[0], "胸软骨占比": float(rate)})

    out = pd.DataFrame(rows, columns=cols_out)
    if out.empty:
        return out
    return (
        out.groupby(["来源", "物料号"], as_index=False)["胸软骨占比"]
        .mean()
        .sort_values(["来源", "物料号"])
