...
try:
    ...
    if overview is None or (hasattr(overview, "empty") and overview.empty):
        ...
        def _read_restore_mapping(uploaded):
            """读取“物料号→目标部位”映射表，支持 xlsx/xls/csv。
            支持可选“比例/权重”列（如 0.4），用于同一物料号拆分为多套还原方案。
            """
            mapping = {}
            if uploaded is None:
                return mapping
            try:
                ...
                for df in frames:
                    ...
                    if not (code_col and tgt_col):
                        continue
                    use_cols = [code_col, tgt_col]
                    if ratio_col:
                        use_cols.append(ratio_col)
                    subset = df[use_cols].dropna(subset=[code_col, tgt_col]).copy()
                    for _, row in subset.iterrows():
                        ...
                        if ratio_col:
                            try:
                                ratio_val = float(row[ratio_col])
                                if ratio_val > 1.0 and ratio_val <= 100.0:
                                    ratio_val = ratio_val / 100.0
                            except Exception:
                                ratio_val = 1.0
                        pieces = [t.strip() for t in re.split(r"[，,;；/|、]+", tgt_raw) if t.strip()]
                        if not pieces:
                            continue
                        entry = {"targets": pieces, "ratio": ratio_val}
        ...
        proj_all = [p for p in (ORDER if 'ORDER' in globals() else sorted(overview["项目"].dropna().unique().tolist())) if p != "总计"]
            ...
            "物料还原映射表（可选上传）",
            type=["xlsx","xls","csv"],
            help="两列：物料号、目标部位；可选增加“比例/权重”列（如 0.4）。多个目标可用逗号/分号/斜杠分隔。",
        ...
        def _calc_for_day(day_value, need_detail=False):
            ...
            def _get_entries(code_key):
                raw = restore_mapping.get(code_key, [])
                if not raw:
                    return []
                if isinstance(raw, list) and len(raw) > 0 and isinstance(raw[0], dict):
                    return raw
                # 兼容旧格式：list[str]
                return [{"targets": raw, "ratio": 1.0}]
            ...
            for _, row in grouped.iterrows():
                ...
                entries = _get_entries(canon_code)
                if not entries:
                    continue
                ratio_sum = 0.0
                for ent in entries:
                    try:
                        r = float(ent.get("ratio", 1.0))
                    except Exception:
                        r = 1.0
                    if r <= 0:
                        continue
                    targets = [_unify(t) for t in ent.get("targets", []) if t]
                ...
                if ratio_sum <= 0:
                    continue
                code_qty[canon_code] = code_qty.get(canon_code, 0.0) + qty * ratio_sum
                code_part[canon_code] = restore_part
                code_ratio_sum[canon_code] = ratio_sum
            bone_rate_total = 0.0
            ...
            for _, row in grouped.iterrows():
                ...
                for ent in entries:
                    ...
                    if use_bone_rule:
                        ...
                        if denom <= 0:
                            continue
                        removed_map[restore_part] = removed_map.get(restore_part, 0.0) + portion_qty
                        for t, adj_rate in adj_rates.items():
                            share = adj_rate / denom
                            inc = portion_qty * share
                    ...
                    if total_rate <= 0:
                        continue
                    removed_map[restore_part] = removed_map.get(restore_part, 0.0) + portion_qty
                    base_qty = portion_qty / total_rate
                    for t in valid_targets:
