...
def _aggregate_sales_rows(df, apply_bb_special=False):
    ...
    if not (dcol and ccol and rev and wt):
        return pd.DataFrame(columns=["日期", "物料号", "销售金额", "销售净重"])
    ...
    if (not apply_bb_special) or (not saty_col) or (not sold_to_col):
...
def _build_sales_detail_rows(xls):
    ...
    for sh in sheets:
        ...
        if saty_col:
            tmp["SaTy_norm"] = _normalize_key_text(df[saty_col]).str.lower()
        else:
            tmp["SaTy_norm"] = ""
        if sold_to_col:
            tmp["售达方_norm"] = _normalize_key_text(df[sold_to_col])
        else:
            tmp["售达方_norm"] = ""
        if not tmp.empty:
            frames.append(tmp)
