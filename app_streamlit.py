...
CHEST_CARTILAGE_CODE = "AEB0630362860"
CHEST_CARTILAGE_CODE_TIELING = "ABB0600322058"
CHEST_CARTILAGE_CODE_BY_SOURCE = {
    "大连": CHEST_CARTILAGE_CODE,
    "DL": CHEST_CARTILAGE_CODE,
    "铁岭": CHEST_CARTILAGE_CODE_TIELING,
    "TL": CHEST_CARTILAGE_CODE_TIELING,
}
CHEST_CARTILAGE_TAX_FACTOR = 1.0  # AEB0630362860 已在 CODES_FACTOR_109 统一乘 1.09
CHEST_RESTORE_META_COLS = [
    "胸软骨还原来源物料号",
    "胸软骨还原胸软骨物料号",
    "胸软骨还原胸软骨占比",
    "胸软骨还原非胸占比",
...
def chest_cartilage_code_for_source(source) -> str:
    s = str(source or "").strip()
    for key, code in CHEST_CARTILAGE_CODE_BY_SOURCE.items():
        if key and key in s:
            return code
    return CHEST_CARTILAGE_CODE


def chest_cartilage_codes_for_rules(rules):
    if rules is None or getattr(rules, "empty", True):
        return [CHEST_CARTILAGE_CODE]
    if "来源" not in rules.columns:
        return [CHEST_CARTILAGE_CODE]
    codes = [chest_cartilage_code_for_source(v) for v in rules["来源"].dropna().astype(str).unique()]
    codes = [str(c).strip() for c in codes if str(c).strip()]
    return sorted(set(codes)) if codes else [CHEST_CARTILAGE_CODE]
...
_SOLD_TO_CANDS = ["售达方","售达方代码","售达方编码","客户代码","客户","Sold-to party","Sold-to"]
_BB_SPECIAL_SOLD_TO = "249666"
_BB_SPECIAL_SATY = {"or", "re", "zcr", "zdr"}
...
def reconcile_month_project_amount_total(project_df, target_total_amount):
    if project_df is None or getattr(project_df, "empty", True):
        return project_df
    if target_total_amount is None or pd.isna(target_total_amount):
        return project_df
    out = project_df.copy()
    if "项目" not in out.columns or "含税金额" not in out.columns:
        return out
    out["含税金额"] = pd.to_numeric(out["含税金额"], errors="coerce").fillna(0.0)
    if "产量(kg)" in out.columns:
        out["产量(kg)"] = pd.to_numeric(out["产量(kg)"], errors="coerce").fillna(0.0)

    dynamic_base = _dynamic_base_projects(out["项目"])
    base_mask = out["项目"].isin(dynamic_base)
    if not base_mask.any():
        return out
    current_total = float(out.loc[base_mask, "含税金额"].sum())
    delta = float(target_total_amount) - current_total
    if abs(delta) <= 1e-6:
        return out

    target_mask = out["项目"].astype(str).str.strip().eq("骨架类")
    if not target_mask.any():
        target_mask = base_mask
    target_idx = out.index[target_mask][0]
    out.loc[target_idx, "含税金额"] = float(out.loc[target_idx, "含税金额"]) + delta
    if "含税单价" in out.columns and "产量(kg)" in out.columns:
        qty = pd.to_numeric(pd.Series([out.loc[target_idx, "产量(kg)"]]), errors="coerce").fillna(0.0).iloc[0]
        out.loc[target_idx, "含税单价"] = (float(out.loc[target_idx, "含税金额"]) / float(qty)) if float(qty) != 0.0 else np.nan

    combo_mask = out["项目"].astype(str).str.strip().eq("鸡头+鸡脖+骨架")
    if combo_mask.any():
        parts = out[out["项目"].isin(["鸡头类", "脖类", "骨架类"])]
        combo_idx = out.index[combo_mask][0]
        if "产量(kg)" in out.columns:
            out.loc[combo_idx, "产量(kg)"] = pd.to_numeric(parts["产量(kg)"], errors="coerce").sum(min_count=1)
        out.loc[combo_idx, "含税金额"] = pd.to_numeric(parts["含税金额"], errors="coerce").sum(min_count=1)
        if "含税单价" in out.columns and "产量(kg)" in out.columns:
            qty = pd.to_numeric(pd.Series([out.loc[combo_idx, "产量(kg)"]]), errors="coerce").fillna(0.0).iloc[0]
            out.loc[combo_idx, "含税单价"] = (float(out.loc[combo_idx, "含税金额"]) / float(qty)) if float(qty) != 0.0 else np.nan
    return out
...
def _normalize_key_text(series):
    return (
        series.astype(str)
        .str.strip()
...
def _is_bb_context(xls):
    names = []
    if isinstance(xls, SheetBundle):
        meta = getattr(xls, "meta", {}) or {}
        for key in ("sale_files", "prod_file", "trans_file", "transfer_file"):
            value = meta.get(key)
    ...
    for name in names:
        low = name.lower()
        if ("bb" in low) or ("蚌埠" in name):
            return True
    return False


def _is_bb_309_name(name) -> bool:
    s = str(name or "").strip()
    low = s.lower()
    # 蚌埠 309 销量文件：A 开头物料按 MvT=309/310 做相反方向处理。
    return ("销量" in s) and ("309" in low)


def _pick_mvt_col(df):
    if df is None or df.empty:
        return None
    cols = [str(c).strip() for c in df.columns]
    for c in ["MvT", "MVT", "mvt", "移动类型"]:
        if c in cols:
            return c
    for c in cols:
        if c.lower() == "mvt":
...
def _normalize_bb_309_310_rows(tmp, source_df, qty_col="销售净重", amt_col="销售金额"):
    """
    BB 309 销量文件行级口径：
    - 仅取 A 开头物料
    - MvT=309 作为正销量/正金额
    - MvT=310 作为负销量/负金额
    """
    if tmp is None or tmp.empty or "__source_file__" not in tmp.columns:
        return tmp
    bb_file_mask = tmp["__source_file__"].map(_is_bb_309_name)
    if not bb_file_mask.any():
        return tmp

    mvt_col = _pick_mvt_col(source_df)
    if not mvt_col:
        return tmp.loc[~bb_file_mask].copy()

    mvt_norm = _normalize_key_text(source_df.loc[tmp.index, mvt_col])
    sign = pd.Series(np.nan, index=tmp.index, dtype="float64")
    sign.loc[mvt_norm.eq("309")] = 1.0
    sign.loc[mvt_norm.eq("310")] = -1.0

    a_material = tmp["物料号"].astype(str).str.startswith("A", na=False)
    eligible = bb_file_mask & a_material & sign.notna()
    tmp = tmp.copy()
    tmp.loc[eligible, qty_col] = pd.to_numeric(tmp.loc[eligible, qty_col], errors="coerce").abs() * sign.loc[eligible]
    tmp.loc[eligible, amt_col] = pd.to_numeric(tmp.loc[eligible, amt_col], errors="coerce").abs() * sign.loc[eligible]
    return tmp.loc[(~bb_file_mask) | eligible].copy()


def _extract_special_sales_rows(df, rev_col=None, wt_col=None):
    """
    兼容新增销量表：
    - 日期：记账日期/记帐日期
    - 物料：仅保留以 A 开头；BB 309 销量文件进一步按 MvT=309 加、MvT=310 减
    - 金额：本币中的外部金额 * 1.09
    - 数量：数量
    仅在常规销量口径列对该行不可用时启用，避免影响原有销量表计算。
    """
    cols = ["日期", "物料号", "销售金额", "销售净重", "SaTy_norm", "售达方_norm"]
    if df is None or df.empty:
        return pd.DataFrame(columns=cols)

    date_col = next((c for c in ["记账日期", "记帐日期"] if c in df.columns), None)
    code_col = "物料" if "物料" in df.columns else None
    amt_col = "本币中的外部金额" if "本币中的外部金额" in df.columns else None
    qty_col = "数量" if "数量" in df.columns else None
    if not (date_col and code_col and amt_col and qty_col):
        return pd.DataFrame(columns=cols)

    tmp = pd.DataFrame({
        "日期": pd.to_datetime(df[date_col], errors="coerce").dt.normalize(),
        "物料号": _normalize_key_text(df[code_col]),
        "销售金额": pd.to_numeric(df[amt_col], errors="coerce") * 1.09,
        "销售净重": pd.to_numeric(df[qty_col], errors="coerce"),
    })
    if "__source_file__" in df.columns:
        tmp["__source_file__"] = df["__source_file__"].astype(str)
        tmp = _normalize_bb_309_310_rows(tmp, df)
        tmp.drop(columns=["__source_file__"], inplace=True, errors="ignore")

    is_309_file = False
    if "__source_file__" in df.columns:
        try:
            is_309_file = df["__source_file__"].astype(str).map(_is_bb_309_name).any()
        except Exception:
            is_309_file = False
    mask = tmp["物料号"].str.startswith("A", na=False)
    # 只有常规“金额+净重”两列都存在时，才把 special 作为补充行，避免重复；
    # 309 类销量文件本身就是 special 口径，不能被“数量列非空”过滤掉。
    if (not is_309_file) and rev_col and wt_col and (rev_col in df.columns) and (wt_col in df.columns):
        mask &= pd.to_numeric(df[rev_col], errors="coerce").isna()
        mask &= pd.to_numeric(df[wt_col], errors="coerce").isna()

    tmp = tmp.loc[mask].dropna(subset=["日期", "物料号"])
    if tmp.empty:
        return pd.DataFrame(columns=cols)
...
def _build_sales_base_rows(df):
    ...
    rev = _pick_col(df.columns, _REV_CANDS)
    wt = _pick_col(df.columns, _NETW_CANDS)

    parts = []
    if dcol and ccol and rev and wt:
        saty_col = next((c for c in df.columns if str(c).strip().lower() == "saty"), None)
        sold_to_col = _pick_col(df.columns, _SOLD_TO_CANDS)
        base = pd.DataFrame({
            "日期": pd.to_datetime(df[dcol], errors="coerce").dt.normalize(),
            "物料号": _normalize_key_text(df[ccol]),
            "销售金额": pd.to_numeric(df[rev], errors="coerce"),
            "销售净重": pd.to_numeric(df[wt], errors="coerce"),
        })

        if "__source_file__" in df.columns:
            base["__source_file__"] = df["__source_file__"].astype(str)
            base = _normalize_bb_309_310_rows(base, df)
            base.drop(columns=["__source_file__"], inplace=True, errors="ignore")
        base = base.dropna(subset=["日期", "物料号"])

        if saty_col:
            base["SaTy_norm"] = _normalize_key_text(df[saty_col]).str.lower()
        else:
            base["SaTy_norm"] = ""

        if sold_to_col:
            base["售达方_norm"] = _normalize_key_text(df[sold_to_col])
        else:
            base["售达方_norm"] = ""

        parts.append(base[cols])

    special = _extract_special_sales_rows(df, rev_col=rev, wt_col=wt)
    if not special.empty:
        parts.append(special[cols])
...
def _aggregate_sales_rows(df, apply_bb_special=False):
    if df is None or df.empty:
        return pd.DataFrame(columns=["日期", "物料号", "销售金额", "销售净重"])

    base = _build_sales_base_rows(df)
    if base.empty:
        return pd.DataFrame(columns=["日期", "物料号", "销售金额", "销售净重"])

    has_saty = ("SaTy_norm" in base.columns) and base["SaTy_norm"].fillna("").astype(str).str.len().gt(0).any()
    has_sold_to = ("售达方_norm" in base.columns) and base["售达方_norm"].fillna("").astype(str).str.len().gt(0).any()

    if (not apply_bb_special) or (not has_saty) or (not has_sold_to):
        filtered = base.loc[~base["SaTy_norm"].isin({"zcr", "zdr"})].copy()
        if filtered.empty:
            return pd.DataFrame(columns=["日期", "物料号", "销售金额", "销售净重"])
        return filtered.groupby(["日期", "物料号"], as_index=False)[["销售金额", "销售净重"]].sum(min_count=1)

    default_rows = base.loc[base["售达方_norm"] != _BB_SPECIAL_SOLD_TO].copy()
    default_rows = default_rows.loc[~default_rows["SaTy_norm"].isin({"zcr", "zdr"})]

    special_rows = base.loc[base["售达方_norm"] == _BB_SPECIAL_SOLD_TO].copy()
    special_other = special_rows.loc[~special_rows["SaTy_norm"].isin(_BB_SPECIAL_SATY)].copy()
    special_other = special_other.loc[~special_other["SaTy_norm"].isin({"zcr", "zdr"})]
    special_target = special_rows.loc[special_rows["SaTy_norm"].isin(_BB_SPECIAL_SATY)].copy()

    parts = []
    if not default_rows.empty:
        parts.append(default_rows.groupby(["日期", "物料号"], as_index=False)[["销售金额", "销售净重"]].sum(min_count=1))
    if not special_other.empty:
        parts.append(special_other.groupby(["日期", "物料号"], as_index=False)[["销售金额", "销售净重"]].sum(min_count=1))

    if not special_target.empty:
        target = (
            special_target.groupby(["日期", "物料号", "SaTy_norm"], as_index=False)[["销售金额", "销售净重"]]
            .sum(min_count=1)
        )
        qty_pivot = (
            target.pivot_table(index=["日期", "物料号"], columns="SaTy_norm", values="销售净重", aggfunc="sum")
            .reindex(columns=["or", "re", "zcr", "zdr"], fill_value=0.0)
            .fillna(0.0)
        )
        amt_pivot = (
            target.pivot_table(index=["日期", "物料号"], columns="SaTy_norm", values="销售金额", aggfunc="sum")
            .reindex(columns=["or", "re", "zcr", "zdr"], fill_value=0.0)
            .fillna(0.0)
        )
        agg = qty_pivot.rename(columns=lambda c: f"{c}_qty").join(
            amt_pivot.rename(columns=lambda c: f"{c}_amt"),
            how="outer"
        ).fillna(0.0)
        a_qty = agg["or_qty"] + agg["re_qty"]
        b_qty = agg["zdr_qty"]
        use_all = a_qty >= b_qty
        qty_all = a_qty + agg["zcr_qty"] + agg["zdr_qty"]
        amt_all = agg["or_amt"] + agg["re_amt"] + agg["zcr_amt"] + agg["zdr_amt"]
        zdr_unit = np.where(agg["zdr_qty"] != 0, agg["zdr_amt"] / agg["zdr_qty"], np.nan)
        qty_pick = np.where(use_all, qty_all, a_qty)
        amt_pick = np.where(use_all, amt_all, zdr_unit * a_qty)
        special_agg = agg.reset_index()[["日期", "物料号"]].copy()
        special_agg["销售金额"] = amt_pick
        special_agg["销售净重"] = qty_pick
        parts.append(special_agg)
    ...
    return (
        ...
        .groupby(["日期", "物料号"], as_index=False)[["销售金额", "销售净重"]]
        .sum(min_count=1)
    )


def _build_sales_detail_rows(xls):
    cols = ["日期", "物料号", "销售金额", "销售净重", "SaTy_norm", "售达方_norm"]
    ...
    frames = []
    for sh in sheets:
        if sh not in _SALES_SHEETS:
            continue
        df = _read_sheet_df(xls, sh)
        if df is None:
            continue
        tmp = _build_sales_base_rows(df)
        if not tmp.empty:
            frames.append(tmp)
    if not frames:
        return pd.DataFrame(columns=cols)
    out = pd.concat(frames, ignore_index=True)
    return out.groupby(["日期", "物料号", "SaTy_norm", "售达方_norm"], as_index=False)[["销售金额", "销售净重"]].sum(min_count=1)


def _build_transfer_detail_rows(xls):
    cols = ["日期", "物料号", "转调金额_含税", "转调数量"]
    sheets = _get_sheet_names(xls)
    if not sheets:
        return pd.DataFrame(columns=cols)
    frames = []
    for sh in sheets:
        if sh not in _TRANSFER_SHEETS:
            continue
        df = _read_sheet_df(xls, sh)
        if df is None:
            continue
        ...
        dcol = _pick_col(df.columns, _DATE_CANDS)
        ccol = _pick_code_col(df)
        ext = _pick_col(df.columns, _EXTAMT_CANDS)
        qty = _pick_col(df.columns, _QTY_CANDS2)
        if not (dcol and ccol and ext and qty):
            continue
        tmp = pd.DataFrame({
            "日期": pd.to_datetime(df[dcol], errors="coerce").dt.normalize(),
            "物料号": _normalize_key_text(df[ccol]),
            "转调金额_含税": pd.to_numeric(df[ext], errors="coerce").abs() * 1.09,
            "转调数量": pd.to_numeric(df[qty], errors="coerce").abs(),
    ...
    if not frames:
        return pd.DataFrame(columns=cols)
    out = pd.concat(frames, ignore_index=True)
    return out.groupby(["日期", "物料号"], as_index=False)[["转调金额_含税", "转调数量"]].sum(min_count=1)


def _merge_asof_price_fill(need_key, fill_key, value_col, out_col=None):
    if out_col is None:
        out_col = value_col
    base_cols = ["日期", "物料号", "月", out_col]
    if need_key is None or need_key.empty:
        return pd.DataFrame(columns=base_cols)
    if fill_key is None or fill_key.empty:
        out = need_key[["日期", "物料号", "月"]].drop_duplicates().copy()
        out[out_col] = np.nan
        return out
    parts = []
    for (code, mon), lk in need_key.groupby(["物料号", "月"], sort=False):
        ...
        if value_col != out_col:
            mk = mk.rename(columns={value_col: out_col})
        parts.append(mk)
    return pd.concat(parts, ignore_index=True) if parts else need_key.assign(**{out_col: np.nan})


def _build_bb_special_mtd_fill(xls, need_key):
    empty = pd.DataFrame(columns=["日期", "物料号", "月", "bb_special_price"])
    if xls is None or need_key is None or need_key.empty or (not _is_bb_context(xls)):
        return empty

    sales = _build_sales_detail_rows(xls)
    if sales.empty:
        return empty
    sales = sales.loc[
        (sales["售达方_norm"] == _BB_SPECIAL_SOLD_TO) &
        (sales["SaTy_norm"].isin(_BB_SPECIAL_SATY))
    ].copy()
    if sales.empty:
        return empty

    sales["月"] = sales["日期"].dt.to_period("M")
    sales_day = sales.groupby(["日期", "物料号", "月", "SaTy_norm"], as_index=False)[["销售金额", "销售净重"]].sum(min_count=1)

    qty_day = (
        sales_day.pivot_table(index=["日期", "物料号", "月"], columns="SaTy_norm", values="销售净重", aggfunc="sum")
        .reindex(columns=["or", "re", "zcr", "zdr"], fill_value=0.0)
        .fillna(0.0)
    )
    amt_day = (
        sales_day.pivot_table(index=["日期", "物料号", "月"], columns="SaTy_norm", values="销售金额", aggfunc="sum")
        .reindex(columns=["or", "re", "zcr", "zdr"], fill_value=0.0)
        .fillna(0.0)
    )
    daily = qty_day.rename(columns=lambda c: f"{c}_qty").join(
        amt_day.rename(columns=lambda c: f"{c}_amt"),
        how="outer"
    ).fillna(0.0).reset_index()

    trans = _build_transfer_detail_rows(xls)
    if not daily.empty and not trans.empty:
        trans = trans.loc[trans["物料号"].isin(daily["物料号"].astype(str).unique())].copy()
    if not trans.empty:
        trans["月"] = trans["日期"].dt.to_period("M")
        trans = trans.groupby(["日期", "物料号", "月"], as_index=False)[["转调金额_含税", "转调数量"]].sum(min_count=1)
        daily = daily.merge(trans, on=["日期", "物料号", "月"], how="outer")

    for col in [
        "or_qty", "re_qty", "zcr_qty", "zdr_qty",
        "or_amt", "re_amt", "zcr_amt", "zdr_amt",
        "转调金额_含税", "转调数量",
    ]:
    ...
    for col in cum_cols:
        daily[f"{col}_cum"] = daily.groupby(["物料号", "月"])[col].cumsum()

    a_qty = daily["or_qty_cum"] + daily["re_qty_cum"]
    b_qty = daily["zdr_qty_cum"]
    use_all = a_qty >= b_qty
    sales_qty_cum = np.where(use_all, a_qty + daily["zcr_qty_cum"] + daily["zdr_qty_cum"], a_qty)
    sales_amt_all = daily["or_amt_cum"] + daily["re_amt_cum"] + daily["zcr_amt_cum"] + daily["zdr_amt_cum"]
    zdr_unit_cum = np.where(daily["zdr_qty_cum"] != 0, daily["zdr_amt_cum"] / daily["zdr_qty_cum"], np.nan)
    sales_amt_cum = np.where(use_all, sales_amt_all, zdr_unit_cum * a_qty)
    total_qty_cum = sales_qty_cum + daily["转调数量_cum"]
    total_amt_cum = sales_amt_cum + daily["转调金额_含税_cum"]
    daily["bb_special_price"] = np.where(total_qty_cum != 0, total_amt_cum / total_qty_cum, np.nan)

    fill_key = daily[["日期", "物料号", "月", "bb_special_price"]].copy()
    need_sub = need_key[["日期", "物料号", "月"]].drop_duplicates().copy()
    return _merge_asof_price_fill(need_sub, fill_key, "bb_special_price")


def _build_bb_special_month_price(xls, date_from, date_to):
    empty = pd.DataFrame(columns=["物料号", "月均价"])
    if xls is None or date_from is None or date_to is None or (not _is_bb_context(xls)):
        return empty

    sales = _build_sales_detail_rows(xls)
    if sales.empty:
        return empty
    sales["日期"] = pd.to_datetime(sales["日期"], errors="coerce").dt.normalize()
    sales = sales.loc[
        (sales["日期"] >= pd.to_datetime(date_from).normalize()) &
        (sales["日期"] <= pd.to_datetime(date_to).normalize()) &
        (sales["售达方_norm"] == _BB_SPECIAL_SOLD_TO) &
        (sales["SaTy_norm"].isin(_BB_SPECIAL_SATY))
    ].copy()
    if sales.empty:
    ...
    sales_sum = sales.groupby(["物料号", "SaTy_norm"], as_index=False)[["销售金额", "销售净重"]].sum(min_count=1)
    qty_sum = (
        sales_sum.pivot_table(index="物料号", columns="SaTy_norm", values="销售净重", aggfunc="sum")
        .reindex(columns=["or", "re", "zcr", "zdr"], fill_value=0.0)
        .fillna(0.0)
    )
    amt_sum = (
        sales_sum.pivot_table(index="物料号", columns="SaTy_norm", values="销售金额", aggfunc="sum")
        .reindex(columns=["or", "re", "zcr", "zdr"], fill_value=0.0)
        .fillna(0.0)
    )
    agg = qty_sum.rename(columns=lambda c: f"{c}_qty").join(
        amt_sum.rename(columns=lambda c: f"{c}_amt"),
        how="outer"
    ).fillna(0.0)
    ...
    b_qty = agg["zdr_qty"]
    use_all = a_qty >= b_qty
    sales_qty = np.where(use_all, a_qty + agg["zcr_qty"] + agg["zdr_qty"], a_qty)
    sales_amt_all = agg["or_amt"] + agg["re_amt"] + agg["zcr_amt"] + agg["zdr_amt"]
    zdr_unit = np.where(agg["zdr_qty"] != 0, agg["zdr_amt"] / agg["zdr_qty"], np.nan)
    sales_amt = np.where(use_all, sales_amt_all, zdr_unit * a_qty)

    trans = _build_transfer_detail_rows(xls)
    if not trans.empty:
        trans["日期"] = pd.to_datetime(trans["日期"], errors="coerce").dt.normalize()
        trans = trans.loc[
            (trans["日期"] >= pd.to_datetime(date_from).normalize()) &
            (trans["日期"] <= pd.to_datetime(date_to).normalize()) &
            (trans["物料号"].isin(agg.index.astype(str)))
        ].copy()
    if not trans.empty:
        trans_sum = trans.groupby("物料号", as_index=False)[["转调金额_含税", "转调数量"]].sum(min_count=1)
        agg = agg.reset_index().merge(trans_sum, on="物料号", how="left").set_index("物料号")
    else:
        agg = agg.reset_index().set_index("物料号")
    if "转调金额_含税" not in agg.columns:
        agg["转调金额_含税"] = 0.0
    if "转调数量" not in agg.columns:
        agg["转调数量"] = 0.0
    agg["转调金额_含税"] = pd.to_numeric(agg["转调金额_含税"], errors="coerce").fillna(0.0)
    agg["转调数量"] = pd.to_numeric(agg["转调数量"], errors="coerce").fillna(0.0)

    total_qty = sales_qty + agg["转调数量"].to_numpy()
    total_amt = sales_amt + agg["转调金额_含税"].to_numpy()
    out = pd.DataFrame({
        "物料号": agg.index.astype(str),
        "月均价": np.where(total_qty != 0, total_amt / total_qty, np.nan),
    })
    return out
...
def build_main_sheet_bundle(prod_file, sale_file, trans_file, transfer_file):
    ...
    meta_key_map = {
        "产量": "prod_file",
        "销量-转调理品原料": "trans_file",
        "销量-调拨宫产量": "transfer_file",
    }
    ...
    for name, upload, optional in targets:
        ...
        if name == "销量" and isinstance(upload, list):
            sale_frames = []
            for up in upload:
                file_name = _get_name(up)
                if file_name:
                    meta["sale_files"].append(file_name)
                xls = _load_excel(up)
                if xls is None:
                ...
                try:
                    ...
                    pass
                df["__source_file__"] = file_name or ""
            ...
            continue
        file_name = _get_name(upload)
        meta_key = meta_key_map.get(name)
        if meta_key and file_name:
    ...
    bundle = SheetBundle(sheets, meta=meta) if sheets else None
...
def _apply_row_min_max_highlight(ws, col_start, col_end, data_start_row, data_end_row):
    if ws is None or data_end_row < data_start_row or col_end < col_start:
        return

    max_fill = PatternFill("solid", fgColor=TREND_MAX_FILL_COLOR)
    min_fill = PatternFill("solid", fgColor=TREND_MIN_FILL_COLOR)
    ...
    for row_idx in range(data_start_row, data_end_row + 1):
        ...
        for col_idx in range(col_start, col_end + 1):
            ...
            if not np.isfinite(numeric_value):
                continue
            numeric_cells.append((cell, numeric_value))
        ...
        if np.isclose(max_value, min_value, rtol=0.0, atol=1e-12):
            continue

        for cell, numeric_value in numeric_cells:
            if np.isclose(numeric_value, max_value, rtol=0.0, atol=1e-12):
                cell.fill = max_fill
            elif np.isclose(numeric_value, min_value, rtol=0.0, atol=1e-12):
                cell.fill = min_fill
...
def _write_core_summary_excel_section(writer, sheet_name, start_row, df_sec, title):
    ...
    for col_idx in range(2, last_col + 1):
        c = ws.cell(row=title_row, column=col_idx)
        c.fill = title_fill
        c.border = border
    ...
    header_cells = {
        ...
        (header_top_row, 8): "屠宰加工\n产量\n(吨)",
        (header_top_row, 9): "外采加工\n产量\n(吨)",
        (header_top_row, 10): "销量\n(吨)",
        (header_top_row, 11): "产销率\n(%)",
        (header_top_row, 12): "宰鸡量\n(千只)",
        (header_top_row, 13): "均重\n(kg/只)",
    ...
    merges = [
        ...
        (header_top_row, 13, header_bottom_row, 13),
    ]
...
def _repair_mojibake_text(value):
    """兼容部分 SAP 导出表头被按 Latin-1/GBK 错读后的乱码列名。"""
    s = str(value).strip()
    if not s:
        return s
    try:
...
def _pick_col_repaired(df, cands):
    """按原列名或修复乱码后的列名匹配候选列，返回原始列名。"""
    if df is None or not hasattr(df, "columns"):
        return None
    cand_set = {str(c).strip() for c in cands}
    for col in df.columns:
        raw = str(col).strip()
        fixed = _repair_mojibake_text(raw)
        if raw in cand_set or fixed in cand_set:
            return col
    return None


def _parse_any_date_series(series):
    s = series.astype(str).str.strip()
    s = s.replace({"0000.00.00": np.nan, "0000-00-00": np.nan, "nan": np.nan, "NaT": np.nan})
    s = s.str.replace(".", "-", regex=False)
    return pd.to_datetime(s, errors="coerce").dt.normalize()


@st.cache_data(show_spinner=False)
def read_external_processing_qty(uploaded):
    """
    读取“外采加工产量”文件，仅用于核心指标的产销率分母与“外采加工产量(吨)”展示。

    支持两类表：
    1) 外采成品量：物料号/短文本/收货日期/收货数量
    2) 产量同格式：物料/物料号 + 日期/记账日期 + 数量/产量
    """
    empty = pd.DataFrame(columns=["日期", "外采加工产量(kg)"])
    if uploaded is None or (isinstance(uploaded, list) and len(uploaded) == 0):
        return empty
    uploads = uploaded if isinstance(uploaded, list) else [uploaded]
    frames = []
    for up in uploads:
        xls = _load_excel(up)
        if xls is None:
            continue
        for sh in _get_sheet_names(xls):
            df = _read_sheet_df(xls, sh)
            if df is None or df.empty:
                continue
            df = df.copy()
            ...
            # 优先识别“外采成品量”口径；否则按“产量”同格式读取。
            dcol = _pick_col_repaired(df, ["收货日期", "日期", "记帐日期", "记账日期", "凭证日期", "过账日期", "单据日期", "输入日期"])
            qcol = _pick_col_repaired(df, ["收货数量", "数量(kg)", "数量", "产量(kg)", "产量", "净重", "净重KG", "净重 KG", "重量", "重量(kg)", "KG", "kg"])
            ccol = _pick_col_repaired(df, ["物料号", "物料", "物料编码", "物料编号", "物料代码", "Material"])
            if not (dcol and qcol):
                continue

            tmp = pd.DataFrame({
                "日期": _parse_any_date_series(df[dcol]),
                "外采加工产量(kg)": pd.to_numeric(df[qcol], errors="coerce"),
            })
            if ccol:
                tmp["物料号"] = _normalize_key_text(df[ccol])
                tmp = tmp[tmp["物料号"].astype(str).str.strip().ne("")]
            tmp = tmp.dropna(subset=["日期", "外采加工产量(kg)"])
            if not tmp.empty:
                frames.append(tmp[["日期", "外采加工产量(kg)"]])

    if not frames:
        return empty
    out = pd.concat(frames, ignore_index=True)
    return out.groupby("日期", as_index=False)["外采加工产量(kg)"].sum(min_count=1)
...
def build_daily_code_price_raw(xls):
    ...
    sale_frames = []
    trans_frames = []
    apply_bb_special = _is_bb_context(xls)
    ...
    for sh in sheets:
        ...
        if sh in _SALES_SHEETS:
            tmp = _aggregate_sales_rows(df, apply_bb_special=apply_bb_special)
            if not tmp.empty:
                sale_frames.append(tmp)
...
def build_daily_total_qty(xls):
    ...
    for sh in sheets:
        ...
        if sh in _SALES_SHEETS:
            tmp = _aggregate_sales_rows(df, apply_bb_special=apply_bb_special)
            if not tmp.empty:
                sale_frames.append(tmp.groupby(["日期"], as_index=False)["销售净重"].sum(min_count=1))
        elif sh in _TRANSFER_SHEETS:
            qty = _pick_col(df.columns, _QTY_CANDS2)
            if not qty:
                continue
...
def fill_price_code_month_avg(pr_raw, qty, code2major, manual_month=None, xls=None):
    """
    ...
    if xls is not None and _is_bb_context(xls):
        bb_special_fill = _build_bb_special_mtd_fill(xls, need_key)
        if bb_special_fill is not None and not getattr(bb_special_fill, "empty", True):
            code_filled = code_filled.merge(
                bb_special_fill.rename(columns={"bb_special_price": "_bb_special_price"}),
                on=["日期", "物料号", "月"],
                how="left"
            )
            code_filled["综合单价_filled"] = code_filled["_bb_special_price"].where(
                code_filled["_bb_special_price"].notna(),
                code_filled["综合单价_filled"]
            )
            code_filled.drop(columns=["_bb_special_price"], inplace=True, errors="ignore")
...
def build_chest_cartilage_price_by_date(pr_raw, dates, manual_month=None, xls=None, chest_codes=None):
    import pandas as _pd
    import numpy as _np

    if dates is None:
        return _pd.DataFrame(columns=["日期", "物料号", "胸软骨单价"])
    date_s = _pd.to_datetime(_pd.Series(list(dates)), errors="coerce").dt.normalize().dropna().drop_duplicates()
    if date_s.empty:
        return _pd.DataFrame(columns=["日期", "物料号", "胸软骨单价"])

    code_s = _pd.Series(chest_codes if chest_codes is not None else [CHEST_CARTILAGE_CODE], dtype="object").astype(str).str.strip()
    code_s = code_s[code_s != ""].drop_duplicates()
    if code_s.empty:
        code_s = _pd.Series([CHEST_CARTILAGE_CODE])
    need = (
        _pd.MultiIndex.from_product([date_s, code_s], names=["日期", "物料号"])
        .to_frame(index=False)
    )
    price = need.copy()
    price["胸软骨单价"] = _np.nan

    if pr_raw is not None and not getattr(pr_raw, "empty", True):
        filled = fill_price_code_month_avg(pr_raw, need, {}, manual_month, xls=xls)
        if filled is not None and not getattr(filled, "empty", True):
            filled = filled.rename(columns={"综合单价_filled": "胸软骨单价"})
            price = price.drop(columns=["胸软骨单价"], errors="ignore").merge(
                filled[["日期", "物料号", "胸软骨单价"]],
                on=["日期", "物料号"],
                how="left",
            )
    ...
    if manual_month is not None and not getattr(manual_month, "empty", True):
        mm = manual_month.copy()
        if "物料号" in mm.columns and "手工单价" in mm.columns:
            mm["物料号"] = mm["物料号"].astype(str).str.strip()
            manual_price = mm[["物料号", "手工单价"]].copy()
            manual_price["手工单价"] = _pd.to_numeric(manual_price["手工单价"], errors="coerce")
            manual_price = manual_price.dropna(subset=["物料号", "手工单价"]).drop_duplicates("物料号", keep="last")
            if not manual_price.empty:
                price = price.merge(
                    manual_price.rename(columns={"手工单价": "_手工单价"}),
                    on="物料号",
                    how="left",
                )
                price["胸软骨单价"] = price["胸软骨单价"].where(
                    price["胸软骨单价"].notna(),
                    price["_手工单价"],
                )
                price = price.drop(columns=["_手工单价"], errors="ignore")

    return price[["日期", "物料号", "胸软骨单价"]]


def _liveweight_by_date(df_lw):
    import pandas as _pd
    if df_lw is None or getattr(df_lw, "empty", True):
        return _pd.DataFrame(columns=["日期", "毛鸡净重(kg)"])
    ...
    if not dcol or not vcol:
        return _pd.DataFrame(columns=["日期", "毛鸡净重(kg)"])
    out = df_lw[[dcol, vcol]].copy()
    out.columns = ["日期", "毛鸡净重(kg)"]
    out["日期"] = _pd.to_datetime(out["日期"], errors="coerce").dt.normalize()
    out["毛鸡净重(kg)"] = _pd.to_numeric(out["毛鸡净重(kg)"], errors="coerce")
    out = out.dropna(subset=["日期", "毛鸡净重(kg)"])
    if out.empty:
        return _pd.DataFrame(columns=["日期", "毛鸡净重(kg)"])
    return out.groupby("日期", as_index=False)["毛鸡净重(kg)"].sum()


def apply_chest_cartilage_structure_restoration(
    m_split,
    m_code_daily,
    chest_cartilage_rules,
    df_lw,
    chest_price_daily,
):
    import pandas as _pd
    ...
    base_cols = ["日期", "项目", "物料号", "产量(kg)", "含税金额"]
    cols = base_cols + CHEST_RESTORE_META_COLS
    if m_split is None or getattr(m_split, "empty", True):
        return _pd.DataFrame(columns=cols)
    if chest_cartilage_rules is None or getattr(chest_cartilage_rules, "empty", True):
        return m_split.copy()
    if m_code_daily is None or getattr(m_code_daily, "empty", True):
        return m_split.copy()

    rules = chest_cartilage_rules.copy()
    ratio_col = "胸软骨占比" if "胸软骨占比" in rules.columns else "胸软骨标准产成率"
    if "物料号" not in rules.columns or ratio_col not in rules.columns:
        return m_split.copy()
    rules["物料号"] = rules["物料号"].astype(str).str.strip()
    for col in ["脖子占比", "里肌占比", "胸软骨占比", "骨架占比", ratio_col]:
        if col not in rules.columns:
            rules[col] = 0.0
        rules[col] = _pd.to_numeric(rules[col], errors="coerce").fillna(0.0)
    if ratio_col != "胸软骨占比":
        rules["胸软骨占比"] = rules[ratio_col]
    if "来源" not in rules.columns:
        rules["来源"] = ""
    rules["胸软骨物料号"] = rules["来源"].apply(chest_cartilage_code_for_source)
    rules = rules.dropna(subset=["物料号", ratio_col])
    rules = rules[rules["胸软骨占比"] > 0]
    missing_bone = (rules["骨架占比"] <= 0) & (
        rules["脖子占比"] + rules["里肌占比"] + rules["胸软骨占比"] < 1.0
    )
    rules.loc[missing_bone, "骨架占比"] = (
        1.0
        - rules.loc[missing_bone, "脖子占比"]
        - rules.loc[missing_bone, "里肌占比"]
        - rules.loc[missing_bone, "胸软骨占比"]
    )
    if rules.empty:
        return m_split.copy()

    base = m_code_daily.copy()
    for c in ["日期", "物料号", "产量(kg)"]:
        if c not in base.columns:
            return m_split.copy()
    base["日期"] = _pd.to_datetime(base["日期"], errors="coerce").dt.normalize()
    base["物料号"] = base["物料号"].astype(str).str.strip()
    base["产量(kg)"] = _pd.to_numeric(base["产量(kg)"], errors="coerce").fillna(0.0)
    if "含税金额" in base.columns:
        base["含税金额"] = _pd.to_numeric(base["含税金额"], errors="coerce")
    else:
        unit = _pd.to_numeric(base.get("综合单价_filled"), errors="coerce")
        base["含税金额"] = base["产量(kg)"] * unit
    base["含税金额"] = base["含税金额"].fillna(0.0)

    prices = chest_price_daily.copy() if chest_price_daily is not None else _pd.DataFrame(columns=["日期", "胸软骨单价"])
    if not prices.empty:
        prices["日期"] = _pd.to_datetime(prices["日期"], errors="coerce").dt.normalize()
        if "物料号" not in prices.columns:
            prices["物料号"] = CHEST_CARTILAGE_CODE
        prices["物料号"] = prices["物料号"].astype(str).str.strip()
        prices["胸软骨单价"] = _pd.to_numeric(prices["胸软骨单价"], errors="coerce")

    source_codes = set(rules["物料号"].astype(str))
    project_price = m_split.copy()
    if not project_price.empty and {"日期", "项目", "物料号", "产量(kg)", "含税金额"}.issubset(project_price.columns):
        project_price["日期"] = _pd.to_datetime(project_price["日期"], errors="coerce").dt.normalize()
        project_price["物料号"] = project_price["物料号"].astype(str).str.strip()
        project_price["产量(kg)"] = _pd.to_numeric(project_price["产量(kg)"], errors="coerce").fillna(0.0)
        project_price["含税金额"] = _pd.to_numeric(project_price["含税金额"], errors="coerce").fillna(0.0)
        project_price = project_price.loc[~project_price["物料号"].isin(source_codes)].copy()
        project_price = (
            project_price[project_price["项目"].isin(["脖类", "里肌类"])]
            .groupby(["日期", "项目"], as_index=False)[["产量(kg)", "含税金额"]]
            .sum()
        )
        project_price["含税单价"] = _np.where(
            project_price["产量(kg)"] != 0,
            project_price["含税金额"] / project_price["产量(kg)"],
            _np.nan,
        )
        project_price_map = {
            (r["日期"], r["项目"]): r["含税单价"]
            for _, r in project_price.dropna(subset=["含税单价"]).iterrows()
        }
    else:
        project_price_map = {}

    source_daily = base[base["物料号"].isin(source_codes)].copy()
    if source_daily.empty:
        return m_split.copy()
    source_daily = (
        source_daily.groupby(["日期", "物料号"], as_index=False)
        .agg({"产量(kg)": "sum", "含税金额": "sum"})
    )
    ratio_cols = ["来源", "胸软骨物料号", "脖子占比", "里肌占比", "胸软骨占比", "骨架占比"]
    source_daily = source_daily.merge(rules[["物料号"] + ratio_cols], on="物料号", how="left")
    source_daily = source_daily.dropna(subset=["胸软骨占比"])
    if source_daily.empty:
        return m_split.copy()

    price_map = {
        (r["日期"], str(r["物料号"]).strip()): r["胸软骨单价"]
        for _, r in prices.dropna(subset=["日期", "物料号", "胸软骨单价"]).iterrows()
    }
    ...
    for d, g in source_daily.groupby("日期"):
        if pd.isna(d):
            continue
        source_qty = _pd.to_numeric(g["产量(kg)"], errors="coerce").fillna(0.0)
        source_amt = _pd.to_numeric(g["含税金额"], errors="coerce").fillna(0.0)
        source_total_qty = float(source_qty.sum())
        if source_total_qty <= 0:
            continue
        for _, row in g.iterrows():
            row_qty = float(row["产量(kg)"])
            chest_code = str(row.get("胸软骨物料号") or CHEST_CARTILAGE_CODE).strip()
            price = price_map.get((d, chest_code), _np.nan)
            if pd.isna(price):
                missing_price_days.append((d, chest_code))
                continue
            neck_ratio = max(float(row.get("脖子占比", 0.0) or 0.0), 0.0)
            tender_ratio = max(float(row.get("里肌占比", 0.0) or 0.0), 0.0)
            chest_ratio = max(float(row.get("胸软骨占比", 0.0) or 0.0), 0.0)
            bone_ratio = max(float(row.get("骨架占比", 0.0) or 0.0), 0.0)
            non_chest_total = neck_ratio + tender_ratio + bone_ratio
            if non_chest_total <= 0:
                bone_ratio = max(1.0 - chest_ratio, 0.0)
                non_chest_total = bone_ratio
            chest_qty = row_qty * chest_ratio
            chest_amt = chest_qty * float(price) * CHEST_CARTILAGE_TAX_FACTOR
            remaining_amt = float(row["含税金额"]) - chest_amt
            code = str(row["物料号"]).strip()
            restore_meta = {
                "胸软骨还原来源物料号": code,
                "胸软骨还原胸软骨物料号": chest_code,
                "胸软骨还原胸软骨占比": chest_ratio,
                "胸软骨还原非胸占比": non_chest_total,
            }
            restored_pairs.add((d, code))
            row_new_rows = []
            split_parts = [
                ("脖类", neck_ratio),
                ("里肌类", tender_ratio),
            ...
            known_amount = 0.0
            pending_parts = []
            for project, part_ratio in split_parts:
                if part_ratio <= 0:
                    continue
                part_qty = row_qty * part_ratio
                unit = project_price_map.get((d, project), _np.nan)
                if project in {"脖类", "里肌类"} and _pd.notna(unit):
                    part_amt = part_qty * float(unit)
                    known_amount += part_amt
                    row_new_rows.append({
                        "日期": d,
                        "项目": project,
                        "物料号": code,
                    ...
                    pending_parts.append((project, part_ratio, part_qty))
            remaining_for_pending = remaining_amt - known_amount
            pending_ratio_total = sum(part_ratio for _, part_ratio, _ in pending_parts)
            for project, part_ratio, part_qty in pending_parts:
                share = part_ratio / pending_ratio_total if pending_ratio_total > 0 else 0.0
                row_new_rows.append({
                    "日期": d,
                    "项目": project,
                    "物料号": code,
                    "产量(kg)": part_qty,
                    "含税金额": remaining_for_pending * share,
                    **restore_meta,
                })
            row_new_rows.append({
                "日期": d,
            ...
            new_rows.extend(row_new_rows)

    if missing_price_days:
        try:
            missing_text = "、".join(sorted({
                f"{pd.Timestamp(d).strftime('%Y-%m-%d')}({code})"
                for d, code in missing_price_days
            }))
            st.warning(f"胸软骨物料缺少价格，以下日期/物料未执行胸软骨占比还原：{missing_text}")
        except Exception:
            pass
    ...
    out = m_split.copy()
    out["日期"] = _pd.to_datetime(out["日期"], errors="coerce").dt.normalize()
    out["物料号"] = out["物料号"].astype(str).str.strip()
    for col in CHEST_RESTORE_META_COLS:
        if col not in out.columns:
            out[col] = _np.nan
    if restored_pairs:
        key = _pd.MultiIndex.from_tuples(restored_pairs, names=["日期", "物料号"])
        row_key = _pd.MultiIndex.from_frame(out[["日期", "物料号"]])
        out = out.loc[~row_key.isin(key)].copy()
    return _pd.concat([out[cols], _pd.DataFrame(new_rows, columns=cols)], ignore_index=True)


def build_month_code_detail_with_restoration_value_conservation(
    minor_month,
    price_code_mtd,
    chest_cartilage_rules=None,
    part_col="项目",
):
    import pandas as _pd
    import numpy as _np

    out_cols = [part_col, "物料号", "产量(kg)", "月均价", "含税金额"]
    if minor_month is None or getattr(minor_month, "empty", True):
        detail = _pd.DataFrame(columns=out_cols)
        detail.attrs["chest_restoration_negative_value_warnings"] = []
        detail.attrs["chest_restoration_missing_part_price_warnings"] = []
        detail.attrs["chest_restoration_conservation_adjustments"] = []
        detail.attrs["chest_restoration_total_target_amount"] = 0.0
        return detail

    work = minor_month.copy()
    if part_col not in work.columns or "物料号" not in work.columns or "产量(kg)" not in work.columns:
        detail = _pd.DataFrame(columns=out_cols)
        detail.attrs["chest_restoration_negative_value_warnings"] = []
        detail.attrs["chest_restoration_missing_part_price_warnings"] = []
        detail.attrs["chest_restoration_conservation_adjustments"] = []
    ...
    work["物料号"] = work["物料号"].astype(str).str.strip()
    work["产量(kg)"] = _pd.to_numeric(work["产量(kg)"], errors="coerce").fillna(0.0)
    detail = work.groupby([part_col, "物料号"], as_index=False)[["产量(kg)"]].sum(min_count=1)

    price = price_code_mtd.copy() if price_code_mtd is not None else _pd.DataFrame(columns=["物料号", "月均价"])
    if "物料号" not in price.columns:
        price["物料号"] = _pd.Series(dtype="object")
    if "月均价" not in price.columns:
        price["月均价"] = _np.nan
    price["物料号"] = price["物料号"].astype(str).str.strip()
    price["月均价"] = _pd.to_numeric(price["月均价"], errors="coerce")
    price = price.dropna(subset=["物料号"]).drop_duplicates("物料号", keep="last")
    detail = detail.merge(price[["物料号", "月均价"]], on="物料号", how="left")
    detail["含税金额"] = detail["月均价"] * detail["产量(kg)"]
    initial_total_amount = float(_pd.to_numeric(detail["含税金额"], errors="coerce").fillna(0.0).sum())
    target_total_delta = 0.0

    warnings = []
    price_map = {
        str(r["物料号"]).strip(): r["月均价"]
        for _, r in price.dropna(subset=["物料号", "月均价"]).iterrows()
    }
    missing_part_price_warnings = []
    missing_part_price_keys = set()
    conservation_adjustments = []
    meta_source_col = "胸软骨还原来源物料号"

    if meta_source_col in work.columns:
        is_restore_row = work[meta_source_col].fillna("").astype(str).str.strip() != ""
    else:
        is_restore_row = _pd.Series(False, index=work.index)
    normal_work = work.loc[~is_restore_row].copy()
    if not normal_work.empty:
        normal_detail = normal_work.groupby([part_col, "物料号"], as_index=False)[["产量(kg)"]].sum(min_count=1)
        normal_detail = normal_detail.merge(price[["物料号", "月均价"]], on="物料号", how="left")
        normal_detail["含税金额"] = normal_detail["月均价"] * normal_detail["产量(kg)"]
        part_price = normal_detail.groupby(part_col, as_index=False).agg({"产量(kg)": "sum", "含税金额": "sum"})
        part_price["部位月均价"] = _np.where(
            _pd.to_numeric(part_price["产量(kg)"], errors="coerce") != 0,
            _pd.to_numeric(part_price["含税金额"], errors="coerce") / _pd.to_numeric(part_price["产量(kg)"], errors="coerce"),
            _np.nan,
        )
        part_price_map = {
            str(r[part_col]).strip(): r["部位月均价"]
            for _, r in part_price.dropna(subset=["部位月均价"]).iterrows()
        }
    else:
        part_price_map = {}

    def _record_missing_part_price(part, source_code, fallback_code):
        key = (str(part), str(source_code), str(fallback_code))
        if key in missing_part_price_keys:
            return
        missing_part_price_keys.add(key)
        missing_part_price_warnings.append({
            "部位": str(part),
            "物料号": str(source_code),
            "兜底物料号": str(fallback_code),
        })

    def _price_for_restored_part(part, source_code, chest_code):
        part = str(part).strip()
        if part in {"胸类", "胸类-胸", "胸类-胸皮"}:
            chest_code = str(chest_code or CHEST_CARTILAGE_CODE).strip()
            chest_price_value = price_map.get(chest_code, _np.nan)
            if _pd.notna(chest_price_value):
                return float(chest_price_value)
            _record_missing_part_price(part, source_code, chest_code)
            return _np.nan
        part_price_value = part_price_map.get(part, _np.nan)
        if _pd.notna(part_price_value):
            return float(part_price_value)
        fallback_code = source_code
        _record_missing_part_price(part, source_code, fallback_code)
        return price_map.get(fallback_code, _np.nan)

    def _add_restored_delta(part, code, restored_qty, desired_amount):
        mask = detail[part_col].astype(str).str.strip().eq(str(part).strip()) & detail["物料号"].astype(str).str.strip().eq(str(code).strip())
        if not mask.any():
            return False
        code_price = price_map.get(str(code).strip(), _np.nan)
        current_amount = 0.0 if _pd.isna(code_price) else restored_qty * float(code_price)
        delta = float(desired_amount) - current_amount
        target_idx = detail.index[mask][0]
        target_amount_before = _pd.to_numeric(_pd.Series([detail.loc[target_idx, "含税金额"]]), errors="coerce").fillna(0.0).iloc[0]
        detail.loc[target_idx, "含税金额"] = float(target_amount_before) + delta
        return True

    def _apply_value_adjustment(source_code, chest_code, chest_ratio, non_chest_ratio, non_chest_qty, restored_chest_qty, restored_rows=None):
        nonlocal target_total_delta
        if non_chest_ratio <= 0 or non_chest_qty <= 0:
            return False
        source_mask = detail["物料号"].astype(str).str.strip().eq(source_code)
        if not source_mask.any():
            return False
        source_price = price_map.get(source_code, _np.nan)
        if _pd.isna(source_price):
            return False
        original_qty = non_chest_qty / non_chest_ratio
        source_original_amount = original_qty * float(source_price)
        assigned_amount = 0.0
        bone_qty = 0.0
        initial_restored_amount = 0.0

        if restored_rows is None or getattr(restored_rows, "empty", True):
            restored_rows = detail.loc[source_mask, [part_col, "物料号", "产量(kg)"]].copy()
            if restored_chest_qty <= 0:
                restored_chest_qty = original_qty * chest_ratio
            if restored_chest_qty > 0:
                restored_rows = _pd.concat([
                    restored_rows,
                    _pd.DataFrame([{part_col: "胸类-胸", "物料号": chest_code, "产量(kg)": restored_chest_qty}]),
                ], ignore_index=True)

        for _, restored_row in restored_rows.iterrows():
            part = str(restored_row.get(part_col) or "").strip()
            code = str(restored_row.get("物料号") or "").strip()
            qty = float(_pd.to_numeric(_pd.Series([restored_row.get("产量(kg)")]), errors="coerce").fillna(0.0).iloc[0])
            if qty == 0 or not part or not code:
                continue
            row_price = price_map.get(code, _np.nan)
            if code in {source_code, chest_code} and _pd.notna(row_price):
                initial_restored_amount += qty * float(row_price)
            if code == source_code and part == "骨架类":
                bone_qty += qty
                continue
            if code not in {source_code, chest_code}:
                continue
            part_month_price = _price_for_restored_part(part, source_code, chest_code)
            if _pd.isna(part_month_price):
                continue
            desired_amount = qty * float(part_month_price)
            if _add_restored_delta(part, code, qty, desired_amount):
                assigned_amount += desired_amount

        residual_bone_amount = source_original_amount - assigned_amount
        target_total_delta += source_original_amount - initial_restored_amount
        if bone_qty > 0:
            target_mask = source_mask & detail[part_col].astype(str).str.strip().eq("骨架类")
            current_bone_amount = bone_qty * float(source_price)
            adjustment = residual_bone_amount - current_bone_amount
        else:
            target_mask = source_mask & detail[part_col].astype(str).str.strip().eq("骨架类")
            adjustment = residual_bone_amount - float(_pd.to_numeric(detail.loc[source_mask, "含税金额"], errors="coerce").fillna(0.0).sum())
        if not target_mask.any():
            max_idx = _pd.to_numeric(detail.loc[source_mask, "产量(kg)"], errors="coerce").fillna(0.0).idxmax()
            target_mask = detail.index == max_idx
        target_idx = detail.index[target_mask][0]
        target_amount_before = _pd.to_numeric(_pd.Series([detail.loc[target_idx, "含税金额"]]), errors="coerce").fillna(0.0).iloc[0]
        detail.loc[target_idx, "含税金额"] = float(target_amount_before) + adjustment
        final_delta = source_original_amount - (assigned_amount + residual_bone_amount)
        if abs(final_delta) > 1e-6:
            detail.loc[target_idx, "含税金额"] = float(detail.loc[target_idx, "含税金额"]) + final_delta
            residual_bone_amount += final_delta
        adjusted_amount = float(detail.loc[target_idx, "含税金额"])
        target_qty = float(_pd.to_numeric(_pd.Series([detail.loc[target_idx, "产量(kg)"]]), errors="coerce").fillna(0.0).iloc[0])
        conservation_adjustments.append({
            "物料号": source_code,
            "胸软骨物料号": chest_code,
            "承接部位": str(detail.loc[target_idx, part_col]),
            "来源原金额": source_original_amount,
            "已分配金额": assigned_amount,
            "骨架承接金额": residual_bone_amount,
            "还原后合计金额": assigned_amount + residual_bone_amount,
            "最终校正差额": final_delta,
        })
        if adjusted_amount < -1e-6:
            warnings.append({
                "物料号": source_code,
                "承接部位": str(detail.loc[target_idx, part_col]),
                "承接产量(kg)": target_qty,
    ...
    meta_adjusted_sources = set()
    if meta_source_col in work.columns:
        meta_work = work.copy()
        meta_work[meta_source_col] = meta_work[meta_source_col].fillna("").astype(str).str.strip()
        meta_work = meta_work[meta_work[meta_source_col] != ""].copy()
        if not meta_work.empty:
            for col in ["胸软骨还原胸软骨占比", "胸软骨还原非胸占比"]:
                if col not in meta_work.columns:
                    meta_work[col] = 0.0
                meta_work[col] = _pd.to_numeric(meta_work[col], errors="coerce").fillna(0.0)
            if "胸软骨还原胸软骨物料号" not in meta_work.columns:
                meta_work["胸软骨还原胸软骨物料号"] = CHEST_CARTILAGE_CODE
            meta_work["胸软骨还原胸软骨物料号"] = meta_work["胸软骨还原胸软骨物料号"].fillna("").astype(str).str.strip()
            meta_group_cols = [
                "胸软骨还原来源物料号",
                "胸软骨还原胸软骨物料号",
                "胸软骨还原胸软骨占比",
            ...
            for meta_key, meta_group in meta_work.groupby(meta_group_cols, dropna=False):
                source_code, chest_code, chest_ratio, non_chest_ratio = meta_key
                source_code = str(source_code).strip()
                chest_code = str(chest_code or CHEST_CARTILAGE_CODE).strip()
                if not source_code:
                    continue
                non_chest_qty = float(_pd.to_numeric(
                    meta_group.loc[meta_group["物料号"].astype(str).str.strip().eq(source_code), "产量(kg)"],
                    errors="coerce",
                ).fillna(0.0).sum())
                restored_chest_qty = float(_pd.to_numeric(
                    meta_group.loc[meta_group["物料号"].astype(str).str.strip().eq(chest_code), "产量(kg)"],
                    errors="coerce",
                ).fillna(0.0).sum())
                if _apply_value_adjustment(
                    source_code,
                    chest_code,
                    float(chest_ratio or 0.0),
                    float(non_chest_ratio or 0.0),
                    non_chest_qty,
                    restored_chest_qty,
                    restored_rows=meta_group[[part_col, "物料号", "产量(kg)"]].copy(),
                ):
                    meta_adjusted_sources.add(source_code)

    rules = chest_cartilage_rules.copy() if chest_cartilage_rules is not None else _pd.DataFrame()
    if not getattr(rules, "empty", True) and "物料号" in rules.columns:
        ratio_col = "胸软骨占比" if "胸软骨占比" in rules.columns else "胸软骨标准产成率"
        if ratio_col in rules.columns:
            if "来源" not in rules.columns:
                rules["来源"] = ""
            rules["物料号"] = rules["物料号"].astype(str).str.strip()
            for col in ["脖子占比", "里肌占比", "胸软骨占比", "骨架占比", ratio_col]:
            ...
            if ratio_col != "胸软骨占比":
                rules["胸软骨占比"] = rules[ratio_col]
            if "胸软骨物料号" not in rules.columns:
                rules["胸软骨物料号"] = rules["来源"].apply(chest_cartilage_code_for_source)
            else:
                rules["胸软骨物料号"] = rules["胸软骨物料号"].astype(str).str.strip()
            missing_bone = (rules["骨架占比"] <= 0) & (
                rules["脖子占比"] + rules["里肌占比"] + rules["胸软骨占比"] < 1.0
            )
            rules.loc[missing_bone, "骨架占比"] = (
            ...
            for _, rule in rules.iterrows():
                source_code = str(rule.get("物料号") or "").strip()
                if not source_code:
                    continue
                if source_code in meta_adjusted_sources:
                    continue
                chest_ratio = max(float(rule.get("胸软骨占比", 0.0) or 0.0), 0.0)
                if chest_ratio <= 0:
                    continue
                neck_ratio = max(float(rule.get("脖子占比", 0.0) or 0.0), 0.0)
                tender_ratio = max(float(rule.get("里肌占比", 0.0) or 0.0), 0.0)
                bone_ratio = max(float(rule.get("骨架占比", 0.0) or 0.0), 0.0)
                non_chest_ratio = neck_ratio + tender_ratio + bone_ratio
                if non_chest_ratio <= 0:
                    non_chest_ratio = max(1.0 - chest_ratio, 0.0)
                if non_chest_ratio <= 0:
                    continue
                source_mask = detail["物料号"].astype(str).str.strip().eq(source_code)
                if not source_mask.any():
                    continue
                chest_code = str(rule.get("胸软骨物料号") or CHEST_CARTILAGE_CODE).strip()
                non_chest_qty = float(_pd.to_numeric(detail.loc[source_mask, "产量(kg)"], errors="coerce").fillna(0.0).sum())
                if non_chest_qty <= 0:
                    continue
                _apply_value_adjustment(
                    source_code,
                    chest_code,
                    chest_ratio,
                    non_chest_ratio,
    ...
    detail["月均价"] = _np.where(
        _pd.to_numeric(detail["产量(kg)"], errors="coerce") != 0,
        _pd.to_numeric(detail["含税金额"], errors="coerce") / _pd.to_numeric(detail["产量(kg)"], errors="coerce"),
        _np.nan,
    )
    result = detail[out_cols].copy()
    result.attrs["chest_restoration_negative_value_warnings"] = warnings
    result.attrs["chest_restoration_missing_part_price_warnings"] = missing_part_price_warnings
    result.attrs["chest_restoration_conservation_adjustments"] = conservation_adjustments
    result.attrs["chest_restoration_total_target_amount"] = initial_total_amount + target_total_delta
    return result


def warn_chest_restoration_month_value_issues(detail):
    warnings = getattr(detail, "attrs", {}).get("chest_restoration_negative_value_warnings", []) if detail is not None else []
    missing_part_warnings = getattr(detail, "attrs", {}).get("chest_restoration_missing_part_price_warnings", []) if detail is not None else []
    try:
        if missing_part_warnings:
            rows = []
            for item in missing_part_warnings[:10]:
                rows.append(
                    f"{item.get('物料号')}->{item.get('部位')}: 兜底物料 {item.get('兜底物料号')}"
                )
            more = f"；另有 {len(missing_part_warnings) - 10} 条" if len(missing_part_warnings) > 10 else ""
            st.warning("胸软骨月累计还原缺少对应部位月均价，已使用兜底物料月均价：" + "；".join(rows) + more)
        if warnings:
            rows = []
            for item in warnings[:10]:
                rows.append(
                    f"{item.get('物料号')}({item.get('承接部位')}): "
                    f"调整后金额 {float(item.get('调整后金额', 0.0)):.2f}, "
                    f"来源原金额 {float(item.get('来源原金额', 0.0)):.2f}, "
                    f"已分配金额 {float(item.get('已分配金额', 0.0)):.2f}"
                )
            more = f"；另有 {len(warnings) - 10} 条" if len(warnings) > 10 else ""
            st.warning("胸软骨还原后存在来源骨架剩余金额为负的情况，请检查标准产成率或胸软骨价格：" + "；".join(rows) + more)
    except Exception:
        pass


def build_overview(xls, code2major, df_lw=None, manual_month_df=None, alloc_long=None, chest_cartilage_rules=None):
    # 产量
    ...
    pr_fill = fill_price_code_month_avg(pr_raw, qty, code2major, manual_month_df, xls=xls)

    m = qty.merge(pr_fill, on=["日期","物料号"], how="left")

    # << 部位分摊：若命中分摊规则则按比例拆分，否则回落到一对一映射 >>
    m_split = apply_part_allocation(m, alloc_long, code2major)
    if chest_cartilage_rules is not None and not getattr(chest_cartilage_rules, "empty", True):
        chest_price_daily = build_chest_cartilage_price_by_date(
            pr_raw,
            qty["日期"].drop_duplicates(),
            manual_month_df,
            xls=xls,
            chest_codes=chest_cartilage_codes_for_rules(chest_cartilage_rules),
        )
        m_split = apply_chest_cartilage_structure_restoration(
            m_split,
            m,
            chest_cartilage_rules,
            df_lw,
    ...
    minor_group_cols = ["日期","部位大类","子类"]
    present_restore_meta_cols = [c for c in CHEST_RESTORE_META_COLS if c in minors.columns]
    if present_restore_meta_cols:
        for c in present_restore_meta_cols:
            if c.endswith("物料号"):
                minors[c] = minors[c].fillna("").astype(str).str.strip()
            else:
                minors[c] = pd.to_numeric(minors[c], errors="coerce")
        minor_group_cols += present_restore_meta_cols
    minors = minors.groupby(minor_group_cols, as_index=False, dropna=False).agg({"产量(kg)":"sum","含税金额":"sum"})
...
@st.cache_data(show_spinner=False)
def read_chest_cartilage_structure_ratio(uploaded):
    """
    读取胸软骨占比表：
    ['来源','物料号','脖子占比','里肌占比','胸软骨占比','骨架占比']。
    胸软骨占比表示带胸软骨骨架 SKU 内部的胸软骨结构占比。
    """
    import pandas as pd, io as _io

    ratio_out_cols = ["脖子占比", "里肌占比", "胸软骨占比", "骨架占比"]
    cols_out = ["来源", "物料号"] + ratio_out_cols
    if uploaded is None:
    ...
    def _ratio_value(v):
        ...
        try:
            ...
            return None
        if ("%" in s) or out > 1.001:
            out = out / 100.0
        return out if out > 0 else None

    def _find_header(raw_df):
        for i in range(len(raw_df)):
            vals = [str(v).strip() for v in raw_df.iloc[i].tolist()]
            if any(v in {"物料号", "物料号码", "物料编码", "物料"} for v in vals):
                return i
        return None

    def _normalize_sheet(raw_df):
        header_idx = _find_header(raw_df)
        if header_idx is None:
            return pd.DataFrame()
        cols = [str(c).strip() if not pd.isna(c) else "" for c in raw_df.iloc[header_idx].tolist()]
    ...
    try:
        if name.endswith(".csv"):
            frames.append((_get_name(uploaded), _normalize_sheet(pd.read_csv(_io.BytesIO(raw), header=None))))
        else:
            xls = pd.ExcelFile(_io.BytesIO(raw))
            for sh in xls.sheet_names:
                df_raw = pd.read_excel(xls, sheet_name=sh, header=None)
                if df_raw is not None and not df_raw.empty:
                    frames.append((str(sh).strip(), _normalize_sheet(df_raw)))
    except Exception:
        return pd.DataFrame(columns=cols_out)
    ...
    ratio_col_map = {
        "脖子占比": ["脖子占比", "脖占比"],
        "里肌占比": ["里肌占比", "里脊占比"],
        "胸软骨占比": ["胸软骨占比", "胸软骨标准产成率", "标准产成率", "胸软骨产成率", "胸软骨标准率"],
        "骨架占比": ["骨架占比"],
    }
    code_cols = ["物料号", "物料号码", "物料编码", "物料编号", "物料代码", "Material", "物料"]
    ...
    for source, df in frames:
        ...
        if not ccol:
            continue
        found_cols = {
            out_col: next((c for c in aliases if c in df.columns), None)
            for out_col, aliases in ratio_col_map.items()
        }
        if not any(found_cols.values()):
            continue
        for _, r in df.iterrows():
            code_values = normalize_code([r.get(ccol)])
            if not code_values:
                continue
            ratios = {}
            for out_col, raw_col in found_cols.items():
                ratios[out_col] = _ratio_value(r.get(raw_col)) if raw_col else 0.0
                if ratios[out_col] is None:
                    ratios[out_col] = 0.0
            if ratios["胸软骨占比"] <= 0:
                continue
            if ratios["骨架占比"] <= 0 and (ratios["脖子占比"] + ratios["里肌占比"] + ratios["胸软骨占比"]) < 1.0:
                ratios["骨架占比"] = 1.0 - ratios["脖子占比"] - ratios["里肌占比"] - ratios["胸软骨占比"]
            rows.append({"来源": source, "物料号": code_values[0], **{k: float(v) for k, v in ratios.items()}})

    out = pd.DataFrame(rows, columns=cols_out)
    if out.empty:
        return out
    return (
        out.groupby(["来源", "物料号"], as_index=False)[ratio_out_cols]
        .mean()
        .sort_values(["来源", "物料号"])
...
def read_chest_cartilage_standard_yield(uploaded):
    """兼容旧调用名；当前按胸软骨结构占比读取。"""
    return read_chest_cartilage_structure_ratio(uploaded)


def merge_allocation_rules(primary_alloc, fallback_alloc):
    """原部位分摊表优先；fallback 分摊表仅补充未覆盖物料。"""
    import pandas as pd
    cols = ["日期", "物料号", "项目", "权重"]
    primary = primary_alloc if primary_alloc is not None else pd.DataFrame(columns=cols)
    fallback = fallback_alloc if fallback_alloc is not None else pd.DataFrame(columns=cols)
    if getattr(primary, "empty", True):
        return fallback.copy() if fallback is not None else pd.DataFrame(columns=cols)
    if getattr(fallback, "empty", True):
        return primary.copy()
    covered = set(primary["物料号"].dropna().astype(str).str.strip())
    fallback_use = fallback.loc[~fallback["物料号"].astype(str).str.strip().isin(covered)].copy()
    return pd.concat([primary.copy(), fallback_use], ignore_index=True)
...
def _combine_restored_qty_map(qty_map):
    ...
    if "\u80f8\u7c7b" not in out and any(p in out for p in chest_parts):
        out["\u80f8\u7c7b"] = sum(out.get(p, 0.0) for p in chest_parts)
    combo_parts = ["\u9e21\u5934\u7c7b", "\u8116\u7c7b", "\u9aa8\u67b6\u7c7b"]
    if "\u9e21\u5934+\u9e21\u8116+\u9aa8\u67b6" not in out and any(p in out for p in combo_parts):
        out["\u9e21\u5934+\u9e21\u8116+\u9aa8\u67b6"] = sum(out.get(p, 0.0) for p in combo_parts)
    bone_neck_parts = ["\u9aa8\u67b6\u7c7b", "\u8116\u7c7b"]
    if "\u9aa8\u67b6+\u9e21\u8116" not in out and any(p in out for p in bone_neck_parts):
...
def _compute_restored_main_side_qty_amt_for_period(overview, minors, df_lw, restore_mapping, start, end):
    ...
    if "\u80f8\u7c7b" in qty_map:
        chest_delta = sum(float(delta_acc.get(proj, 0.0)) for proj in ("\u80f8\u7c7b-\u80f8", "\u80f8\u7c7b-\u80f8\u76ae"))
        restored_qty_map["\u80f8\u7c7b"] = float(qty_map.get("\u80f8\u7c7b", 0.0)) + chest_delta
...
chest_cartilage_ratio_file = st.sidebar.file_uploader("胸软骨占比表（可选上传）", type=["xlsx","xls","csv"])
...
external_proc_qty_df = read_external_processing_qty(external_proc_upload)
if external_proc_upload and (external_proc_qty_df is None or external_proc_qty_df.empty):
    st.warning("已上传“外采加工产量”文件，但未识别到日期/数量列（支持：收货日期+收货数量，或产量表同格式）。")
...
chest_cartilage_rules = read_chest_cartilage_structure_ratio(chest_cartilage_ratio_file)
...
overview, minors = build_overview(xls, code2major, df_lw, manual_month_df, alloc_long, chest_cartilage_rules)
...
if overview is None or overview.empty:
    ...
    try:
        ...
        def _summary_row(label, start, end):
            ...
            dynamic_base = _dynamic_base_projects(seg["\u9879\u76ee"])
            tot_qty, tot_amt, rate, unit = _segment_metrics(seg, dynamic_base, lw_sum)
            ...
            ext_qty = np.nan
            try:
                if external_proc_qty_df is not None and not external_proc_qty_df.empty:
                    _ext = external_proc_qty_df.copy()
                    _ext["\u65e5\u671f"] = pd.to_datetime(_ext["\u65e5\u671f"], errors="coerce").dt.normalize()
                    _ext_mask = (_ext["\u65e5\u671f"] >= start) & (_ext["\u65e5\u671f"] <= end)
                    ext_qty = pd.to_numeric(_ext.loc[_ext_mask, "\u5916\u91c7\u52a0\u5de5\u4ea7\u91cf(kg)"], errors="coerce").sum(min_count=1)
            except Exception:
                ext_qty = np.nan
            sale_rate_qty = (float(tot_qty) if pd.notna(tot_qty) else 0.0) + (float(ext_qty) if pd.notna(ext_qty) else 0.0)
            sale_rate = (tot_sale / sale_rate_qty * 100.0) if (sale_rate_qty != 0 and pd.notna(tot_sale)) else np.nan
            ...
            def _row_value(item_name, col_name):
                row = main_side_rows.loc[main_side_rows["\u9879\u76ee"] == item_name, col_name]
                return row.iloc[0] if not row.empty else np.nan

            main_qty = _row_value("\u4e3b\u4ea7\u54c1", "\u4ea7\u91cf(kg)")
            side_qty = _row_value("\u526f\u4ea7\u54c1", "\u4ea7\u91cf(kg)")
            main_unit = _row_value("\u4e3b\u4ea7\u54c1", "\u542b\u7a0e\u5355\u4ef7")
            side_unit = _row_value("\u526f\u4ea7\u54c1", "\u542b\u7a0e\u5355\u4ef7")
            is_current_month_cum = (pd.Timestamp(start) == pd.Timestamp(month_start) and pd.Timestamp(end) == pd.Timestamp(sel_dt))
            if pd.notna(tot_qty) and pd.notna(main_qty):
                side_qty = float(tot_qty) - float(main_qty)
            if is_current_month_cum and pd.notna(unit) and pd.notna(main_unit):
                side_unit = float(unit) - float(main_unit)
            ...
            return {
                ...
                "\u5c60\u5bb0\u52a0\u5de5\u4ea7\u91cf(\u5428)": (tot_qty / 1000.0) if pd.notna(tot_qty) else np.nan,
                "\u5916\u91c7\u52a0\u5de5\u4ea7\u91cf(\u5428)": (ext_qty / 1000.0) if pd.notna(ext_qty) else np.nan,
        ...
        summary_order = [
            "日期",
            "主产品产成率(%)", "副产品产成率(%)", "产成率(%)",
            "主产品产值(元/kg)", "副产品产值(元/kg)", "产值(元/kg)",
            "屠宰加工产量(吨)", "外采加工产量(吨)", "销量(吨)", "产销率(%)",
            "宰鸡量(千只)", "均重(kg/只)",
        ]
        ...
        st.markdown("### 核心指标概览")
        core_summary_placeholder = st.empty()
        core_summary_placeholder.dataframe(summary_disp, use_container_width=True)
    ...
    with st.expander("累计（本月至所选日）", expanded=True):
        ...
        try:
            ...
            if xls is not None and _is_bb_context(xls):
                try:
                    bb_price_code_mtd = _build_bb_special_month_price(xls, month_start, sel_dt)
                except Exception:
                    bb_price_code_mtd = pd.DataFrame(columns=["物料号","月均价"])
                if bb_price_code_mtd is not None and not getattr(bb_price_code_mtd, "empty", True):
                    price_code_mtd = price_code_mtd.merge(
                        bb_price_code_mtd.rename(columns={"月均价": "_bb_special_month_price"}),
                        on="物料号",
                        how="outer"
                    )
                    price_code_mtd["月均价"] = price_code_mtd["_bb_special_month_price"].where(
                        price_code_mtd["_bb_special_month_price"].notna(),
                        price_code_mtd["月均价"]
                    )
                    price_code_mtd = price_code_mtd.drop(columns=["_bb_special_month_price"], errors="ignore")
            # 补价表兜底月均价
        ...
        # —— 当月产量（已分摊/还原后的子类）× 原物料月均价 → 金额，再按部位汇总 ——
        cum_base = pd.DataFrame()
        chest_restoration_total_target_amount = np.nan
        try:
            if minors is not None and not minors.empty:
                ...
                if not minor_month.empty:
                    ...
                    detail = build_month_code_detail_with_restoration_value_conservation(
                        minor_month,
                        price_code_mtd,
                        chest_cartilage_rules,
                        part_col="项目",
                    )
                    warn_chest_restoration_month_value_issues(detail)
                    chest_restoration_total_target_amount = detail.attrs.get(
                        "chest_restoration_total_target_amount",
                        np.nan,
                    )
        ...
        cum_base = reconcile_month_project_amount_total(
            cum_base,
            chest_restoration_total_target_amount,
        )
        ...
        # Align core summary cumulative values with the cumulative table rows.
        try:
            ...
            if (
                ...
                and {"\u65e5\u671f", "\u4ea7\u503c(\u5143/kg)", "\u4e3b\u4ea7\u54c1\u4ea7\u503c(\u5143/kg)", "\u526f\u4ea7\u54c1\u4ea7\u503c(\u5143/kg)"} <= set(export_core_summary.columns)
            ):
                mask = export_core_summary["\u65e5\u671f"] == month_label
                if mask.any():
                    main_row = export_month_cum[export_month_cum["\u9879\u76ee"] == "\u4e3b\u4ea7\u54c1"]
                    side_row = export_month_cum[export_month_cum["\u9879\u76ee"] == "\u526f\u4ea7\u54c1"]
                    total_row_cum = export_month_cum[export_month_cum["\u9879\u76ee"] == "\u603b\u8ba1"]
                    main_unit = pd.to_numeric(main_row.iloc[0]["\u542b\u7a0e\u5355\u4ef7"], errors="coerce") if (not main_row.empty and "\u542b\u7a0e\u5355\u4ef7" in main_row.columns) else np.nan
                    side_unit = pd.to_numeric(side_row.iloc[0]["\u542b\u7a0e\u5355\u4ef7"], errors="coerce") if (not side_row.empty and "\u542b\u7a0e\u5355\u4ef7" in side_row.columns) else np.nan
                    total_unit = pd.to_numeric(total_row_cum.iloc[0]["\u542b\u7a0e\u5355\u4ef7"], errors="coerce") if (not total_row_cum.empty and "\u542b\u7a0e\u5355\u4ef7" in total_row_cum.columns) else np.nan
                    if pd.notna(main_unit):
                        export_core_summary.loc[mask, "\u4e3b\u4ea7\u54c1\u4ea7\u503c(\u5143/kg)"] = float(main_unit)
                    if pd.notna(side_unit):
                        export_core_summary.loc[mask, "\u526f\u4ea7\u54c1\u4ea7\u503c(\u5143/kg)"] = float(side_unit)
                    if pd.notna(total_unit):
                        export_core_summary.loc[mask, "\u4ea7\u503c(\u5143/kg)"] = float(total_unit)
                    if summary_disp is not None and not summary_disp.empty:
                        disp_mask = summary_disp["\u65e5\u671f"] == month_label
                        if disp_mask.any():
                            if pd.notna(main_unit):
                                summary_disp.loc[disp_mask, "\u4e3b\u4ea7\u54c1\u4ea7\u503c(\u5143/kg)"] = round(float(main_unit), 2)
                            if pd.notna(side_unit):
                                summary_disp.loc[disp_mask, "\u526f\u4ea7\u54c1\u4ea7\u503c(\u5143/kg)"] = round(float(side_unit), 2)
                            if pd.notna(total_unit):
                                summary_disp.loc[disp_mask, "\u4ea7\u503c(\u5143/kg)"] = round(float(total_unit), 2)
                            if core_summary_placeholder is not None:
                                core_summary_placeholder.dataframe(summary_disp, use_container_width=True)
        except Exception:
            pass
            pass
        ...
        if minors is not None and not minors.empty:
            try:
                ...
                if not minor_month.empty:
                    ...
                    code_detail = build_month_code_detail_with_restoration_value_conservation(
                        minor_month,
                        code_price,
                        chest_cartilage_rules,
                        part_col="部位大类",
