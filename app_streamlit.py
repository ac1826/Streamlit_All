...
def fill_price_code_month_avg(pr_raw, qty, code2major, manual_month=None):
    ...
    # === 本月月均价回填：只要本月有任何价格记录，即可回填 ===
    agg = (pr_day.groupby(["物料号","月"], as_index=False)
    ...
    agg["综合单价_filled"] = _np.where(agg["数量"] != 0, agg["金额"]/agg["数量"], _np.nan)
    code_filled = agg[["物料号","月","综合单价_filled"]]

    out = out0.merge(code_filled, on=["物料号","月"], how="left")
...
def build_overview_table_for_day(overview, sel, df_lw, total_qty_df=None, pr_raw=None):
...
if overview is None or overview.empty:
    ...
    try:
        ...
        total_qty_df = None
    disp = build_overview_table_for_day(overview, sel, df_lw, total_qty_df=total_qty_df)
