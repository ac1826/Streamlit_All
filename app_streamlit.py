...
def fill_price_code_month_avg(pr_raw, qty, code2major, manual_month=None):
    ...
    - 若该物料在当月前一日之前也完全没有价，则保持缺价（不再用大类兜底）。
    """
...
sale_upload = st.sidebar.file_uploader("销量", type=["xlsx","xls"])
...
missing_main = [name for name, up in required_main if up is None]
if missing_main:
    ...
    with st.expander("累计（本月至所选日）", expanded=True):
        ...
        try:
            ...
            if pr_daily is not None and not getattr(pr_daily, "empty", True):
                ...
                pr_use["金额"] = pd.to_numeric(pr_use["金额"], errors="coerce").abs()
                pr_use["数量"] = pd.to_numeric(pr_use["数量"], errors="coerce").abs()
...
try:
    ...
    if overview is None or (hasattr(overview, "empty") and overview.empty):
        ...
        # 当日（展示用）
        inc_map_ref, removed_map_ref, detail_ref, lw_val, ov_day_ref = _calc_for_day(ref_day, need_detail=False)
            ...
            if not over_month.empty:
                ...
                if show_projects_m:
                    ...
                    for proj in row_order_m:
                        ...
                        new_qty = max(orig_qty + adj_qty, 0.0)
