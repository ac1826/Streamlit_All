...
def _is_bb_309_name(name) -> bool:
    s = str(name or "").strip()
    low = s.lower()
    # 蚌埠 309 销量文件：只要上传的销量文件名里带“309”，该文件的销量数量按正数口径带入。
...
def _extract_special_sales_rows(df, rev_col=None, wt_col=None):
    ...
    if "__source_file__" in df.columns:
        ...
        bb309_mask = tmp["__source_file__"].map(_is_bb_309_name)
        tmp.loc[bb309_mask, "销售净重"] = tmp.loc[bb309_mask, "销售净重"].abs()
        tmp.loc[bb309_mask, "销售金额"] = tmp.loc[bb309_mask, "销售金额"].abs()
...
def _build_sales_base_rows(df):
    ...
    if dcol and ccol and rev and wt:
        ...
        if "__source_file__" in df.columns:
            ...
            bb309_mask = base["__source_file__"].map(_is_bb_309_name)
            base.loc[bb309_mask, "销售净重"] = base.loc[bb309_mask, "销售净重"].abs()
            base.loc[bb309_mask, "销售金额"] = base.loc[bb309_mask, "销售金额"].abs()
            # 309 销量文件：只保留 A 开头物料；A 开头销量/金额按正值带入，其他普通文件完全不变。
            if bb309_mask.any():
                base = base.loc[(~bb309_mask) | (base["物料号"].astype(str).str.startswith("A", na=False))].copy()
