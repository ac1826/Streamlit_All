...
def build_daily_code_price_raw(xls):
    ...
    for sh in sheets:
        ...
        if sh in _SALES_SHEETS:
            ...
            if not tmp.empty:
                # 先按日期汇总，再取绝对值
                day_sum = tmp.groupby(["日期"], as_index=False)[["转调金额_含税","转调数量"]].sum()
                day_sum["转调金额_含税"] = day_sum["转调金额_含税"].abs()
                day_sum["转调数量"] = day_sum["转调数量"].abs()
                # 与销售表按“日期+物料号”合并所需结构对齐
                day_sum["物料号"] = "__DAY_TOTAL__"
                trans_frames.append(day_sum[["日期","物料号","转调金额_含税","转调数量"]])
...
def build_overview_table_for_day(overview, sel, df_lw, pr_raw=None):
    ...
    if pr_raw is not None and not getattr(pr_raw, "empty", True):
        pr = pr_raw.copy()
        pr["日期"] = pd.to_datetime(pr["日期"], errors="coerce").dt.normalize()
        tot_sale = pd.to_numeric(pr.loc[pr["日期"] == sel_dt, "数量"], errors="coerce").sum(min_count=1)
    else:
...
if overview is None or overview.empty:
    ...
    try:
        ...
        pr_raw_for_summary = None
    disp = build_overview_table_for_day(overview, sel, df_lw, pr_raw=pr_raw_for_summary)
    ...
    with st.expander("累计（本月至所选日）", expanded=True):
        ...
        # 总计-销量：全量销量口径（销售净重 + 调理品数量取绝对值），不依赖部位映射
        if pr_raw_for_summary is not None and not pr_raw_for_summary.empty:
            _mask = (pr_raw_for_summary["日期"] >= month_start) & (pr_raw_for_summary["日期"] <= sel_dt)
            tot_sale = pd.to_numeric(pr_raw_for_summary.loc[_mask, "数量"], errors="coerce").sum(min_count=1)
        else:
            tot_sale = np.nan
